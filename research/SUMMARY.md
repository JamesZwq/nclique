...
gate of its Section 11 ("开始吧"); [spec](r1_skyline_index_20260918/stages/IMPLEMENTATION.md).
...
`chain_index.hpp`) stores the all-size hierarchy in 1.5x-8.4x fewer bytes than
one S tree per size (thirteen graphs, median 3.9x), locates any community in
O(1) (3-62 ns) and lists it faster than a memcpy of the vertex list on 14 of
15 measured points; brute-force verified. r >= 2 closed: the paper's size
forest already is the chain structure (CHAINS.md Section 10). Research
...
| Directory / report | Idea and stage | Outcome / reuse decision |
|---|---|---|
| [r1_skyline_index_20260918](r1_skyline_index_20260918/stages/RESULTS_INDEX.md) | All-size community/value index for r=1 adapted from the SIGMOD 2026 SGL bi-component index: dominance through iterated Kruskal-Katona shadows, per-size canonical merge trees with one cross-size pointer per node, one entry per class per skyline size, certified tail = one entry, twin quotient; [theory](r1_skyline_index_20260918/THEORY.md), [counts](r1_skyline_index_20260918/stages/RESULTS.md), then both designs implemented in memory and measured | Proofs plus counts plus implementation. Stage 1: Release/ASan selftests match brute-force nuclei on 34,075 graphs; F2 2,994,580 checks, 0 violations on five real graphs. Stage 2: both designs match brute force (605,476 community queries, 3.65M membership, 1.44M values) and each other on every timed query. Measured bytes (class layout): skyline smaller by 1.11x GrQc, 2.28x HepPh, 1.28x dblp, 1.17x Stanford, 0.99x amazon (with values 1.09/1.84/1.20/1.11/1.00); per-vertex layout computed from the same counts 1.14x-3.88x. Latency: skyline 2.1-2.9x slower on community listing (per output vertex 3.8-5.8 ns vs 1.6-2.3 ns), 1.0-4.4x slower on membership, equal on values. A space/time trade, never both; S trees with DFS intervals remain the fastest exact representation. No production, paper or promotion. 2026-09-19 [chains](r1_skyline_index_20260918/CHAINS.md): hierarchy-equivalence classes (same own node at every size) are a partition refined by twins and coarser by 6.4x-23.6x on the five inputs (dblp 317,080 vertices -> 13,459 chains); every nucleus is a union of chains (proved); projected S trees over chains 2.4x-4.7x smaller than per-vertex S trees, 3.8x-8.1x with aligned labels, at S-tree query speed; counts only, chain index not yet built. MEASURED 2026-09-19 ([results](r1_skyline_index_20260918/stages/RESULTS_CHAINS.md)): S trees over chains vs S trees over twins, with values: GrQc 2.48x, HepPh 3.88x, dblp 3.80x, Stanford 4.53x, amazon 2.97x smaller (aligned-label projection 3.9x-8.3x; vs naive dense 2.7x-18x), community listing 1.7x-8.6x FASTER (0.26-0.40 ns per output vertex), membership up to 3.1x faster, brute-force checked (1,210,952 queries, both modes, Release+ASan); skyline over chains is dominated. Both smaller and faster: the SGL effect, obtained from a partition instead of mu-fold units. FINAL MODULE 2026-09-19 ([RESULTS_FINAL.md](r1_skyline_index_20260918/RESULTS_FINAL.md), `chain_index.hpp`, CHAINS.md Section 9 Lemmas C4-C6): aligned labels + lexicographic chain ranks + per-size run arrays with node entry points; brute-force checked in build, compact and loaded forms (1,816,701 community queries, 10.96M membership checks, Release+ASan); bytes vs per-vertex S trees 3.98x GrQc, 5.70x HepPh, 7.81x dblp, 6.08x Stanford, 3.73x amazon (2.9x-5.0x if the 4n-byte label permutation is charged); community located in O(1) (3-40 ns), range answers 26-660 ns, explicit ids 0.6x-1.65x memcpy speed (faster on the three large graphs, 0.08-0.19 ns per vertex), one range per (2,k)-community by construction; membership 5-18 ns, values 9-18 ns; build 0.02-4.8 s one thread including the all-size peel. Compact run form costs 2-10 percent more bytes than chain-id arrays and answers 5x-53x faster. 13-GRAPH RUN (same day, `final.json`, adds AstroPh, CondMat, cit-HepPh, Brightkite, Epinions, Slashdot, youtube, pokec): bytes 1.47x (cit-HepPh, 2.1 vertices/chain) to 8.42x (youtube, 26.5 vertices/chain) below per-vertex S trees, median 3.9x, 1.39x-4.98x with the permutation; locate 3-62 ns; explicit listing 0.06-0.13 ns/vertex on eleven graphs, 0.20-0.24 on AstroPh and cit-HepPh (branchless fill, `final.json`; the scalar-tail fill of `final_v2.json` was 1.3x-2.4x slower there), faster than the memcpy S-tree listing on 14 of 15 measured points (0.90x-2.02x); chains > canonical nodes on four graphs (pokec 5.7x), refuting the earlier regularity; build 14 ms-67 s, peak RSS 5.1 GB on pokec (build side). r >= 2 closed by accounting (CHAINS.md Section 10): the paper's size forest already is the chain structure. LATER 2026-09-19 (RESULTS_FINAL.md Section 14, `buildonly.json`): build streams solver rows (two-row window) + prefix-trie chain refinement + width from the solver's rows: index byte-identical, peak build RSS dblp 899 -> 157 MB, youtube 1,663 -> 557 MB, pokec 6,100 -> 2,889 MB (floor = solver row index 1.46 GB + graph), build time unchanged within noise; per-size byte widths for tops and residues (CHAINX03): index 12-35 percent smaller again, now 2.0x (cit-HepPh) to 11.2x (youtube) below per-vertex S trees, dblp 10.3x; top packing measured in-process (13 graphs, `final.json`): packed climb 0.91x/0.89x of T-tops climb in the median, i.e. not slower; all latency classes within 0.92-1.12x of the fixed-width run (both signs = noise); locate 3.5-45 ns, listing 0.04-0.28 ns/vertex, faster than memcpy on 13/15 points; build incl. row index 15 ms-34 s. FOUR MORE GRAPHS (`more.json`; omega/sigma widened to 16 bits after dblp-coauthor s_max 450 broke the byte fields): dblp-coauthor 4.05M vertices / 512-bit counts = 11.71x (45 MB vs 530 MB, locate 14 ns, 1.9M-vertex communities listed in 167 us, build 189 s / 8.8 GB peak of which ~4 GB is the solver's 512-bit weight arrays), com-amazon 4.95x, ca-HepTh 5.54x, email-Eu-core 1.28x = worst case (1,005 vertices, 1.5 per chain); 17 graphs: 1.28x-11.71x, median 4.95x. SERVERS 2026-09-19 (tods1 20 graphs, tods2 7; RESULTS_FINAL.md Section 15): 29 distinct graphs 1.28x-48.69x, median 7.0x; web-uk-2005 48.7x (155 vertices/chain), ca-coauthors-dblp 45.8x, web-NotreDame 24.7x, web-BerkStan 19.2x, wiki-Talk 15.9x, web-it-2004 13.8x, cit-Patents 12.1x; index independent of edge order, laptop/server bytes within 0.2%; solver 32-bit member/row limits lifted to 64-bit, but com-lj (350 GB, bad_alloc), hollywood (435 GB, OOM) and orkut exceed the terminal solver's clique-tree memory on a 503 GB machine. VALUES AS DOUBLE (user: precision not needed; CHAINX05; solver still peels exactly, converts once): 64-bit graphs byte-identical value blocks, HepPh -10%, dblp-coauthor -2% (11.97x); the per-size widths had already removed most wide fields; 17-graph rerun `final.json`/`more.json`, all latency classes within noise of the integer run (median ratios 0.6-1.16); exact below 2^53 |
