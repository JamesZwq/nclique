...
| Directory / report | Idea and stage | Outcome / reuse decision |
|---|---|---|
| [r1_skyline_index_20260918](r1_skyline_index_20260918/RESULTS.md) | All-size community/value index for r=1 adapted from the SIGMOD 2026 SGL bi-component index: dominance through iterated Kruskal-Katona shadows, per-size canonical merge trees with one cross-size pointer per node, one entry per class per skyline size, certified tail = one entry, twin quotient; [theory](r1_skyline_index_20260918/THEORY.md) then a counting gate only | Proofs (F1-F8, L1-L4, Theorem Q2, construction) plus counts. Release and ASan/UBSan selftests pass on 34,075 graphs / 844,230 cells / 93,057 brute-force partitions; independent Python verifier 1,501 graphs, 58,924 retrieval queries, no violation; on the five real graphs F2 2,994,580 checks with 0 violations, L1/L2 on every active cell. Storage model (words): skyline smaller than S explicit trees on all five, GrQc 1.25x, HepPh 3.34x, dblp 1.77x, Stanford 1.35x, amazon 1.09x (marginal); membership entries shrink 1.7x-8.3x; residue cells 3.5-6.7% on collaboration graphs but 43-54% on Stanford/amazon; empty canonical nodes 56-75%. No index code, no query or build timing, no promotion |
