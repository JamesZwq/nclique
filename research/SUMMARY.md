...
| Directory / report | Idea and stage | Outcome / reuse decision |
|---|---|---|
| [r1_skyline_index_20260918](r1_skyline_index_20260918/THEORY.md) | Theory only: all-size community/value index adapted from the SIGMOD 2026 SGL bi-component index; dominance order through iterated Kruskal-Katona shadows, per-size merge trees with one cross-size container pointer per canonical node, one entry per class per skyline size, certified tail collapses to a single entry, twin quotient | Proofs of the structural facts, four lemmas, query correctness and construction; exact break-even against S explicit trees (membership shrinks by avg_traj/mu, estimated 1.1x-5.5x from old saturation data; canonical-node count N_T unmeasured). No code, no measurement, no promotion; next step is the four counts of its Section 11 |
