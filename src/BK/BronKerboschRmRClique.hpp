...
namespace bkRmClique {
    ...
    inline void printBitset(const Bitset &bs, std::string name = "") {// BronKerboschFns.hpp
        if (!name.empty()) {
            std::cout << name << ": ";
        }
    ...
    void pathSplit(int n,
        ...
        if (P.count() < minK || P.count() - pivots.count() > minK) {
            return;
        }

        if (P.count() - pivots.count() == minK) {
            Bitset reportPivots(n);
            reportPivots.reset(); // 初始化为全 false
            report(P, reportPivots); // 直接报告当前的 P 和 pivots
            return;
        }
        ...
        if (noConflict) { // 没有冲突，直接运行原来的 bk_run
            if (P.count() >= minK) {
                       ...
                       ReportFn &&report)
    {
        ...
        Bitset P(n); P.set();      // 起始 P 为全部顶点（或 ~staticV 按需剪枝）
    ...
    inline void testBronKerbosch() {
        ...
        int minK = 3; // 最小 clique 大小
        std::vector<double> cliqueCounts(vList.size(), 0);
        removeRClique(vList, conflictSets, minK,
