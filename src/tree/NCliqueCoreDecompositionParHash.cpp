...
void countingPerVertexHelp(const TreeNode &node,
                           ...
                           MutexStaticVector<daf::Size>& core,
...
daf::Size *countingPerVertex(const MultiBranchTree &tree, const daf::CliqueSize k) {
    ...
    auto core = new MutexStaticVector<daf::Size>(tree.getRoot()->children.size());
    //init 0
...
void computeSupHelp(const TreeNode &node,
                           const daf::CliqueSize k,
                           const daf::Size *core,
                           MutexStaticVector<daf::Size> &newCore,
                           StaticVector<daf::Size> &povit,
                           StaticVector<daf::Size> &keepC,
                           std::vector<ThreadSafeMap> &sup
) {
    daf::Size cliqueSize = povit.size() + keepC.size();
    if (node.children.empty() && cliqueSize >= k && keepC.size() <= k) {
        assert(!keepC.empty());
        ...
        if (needPivot == 0) {
            for (const auto v: keepC) {
                ...
                newCore.setIfBigger(v,sup[v].add(maxK, 1));
            }
            return;
        }
        ...
        for (daf::Size i = numGraterThanMaxKinKeepC; i <= orderPovit.size(); i++) {
            if (i == orderPovit.size() || core[orderPovit[i]] < prveCore) {
                ...
                for (const auto v: keepC) {
                    newCore.setIfBigger(v,sup[v].add(prveCore, keepCliqueCount - prveKeepCliqueCount));
                }
                for (daf::Size v = 0; v < prvePovit; v++) {
                    newCore.setIfBigger(orderPovit[v],sup[orderPovit[v]].add(prveCore, povitCliqueCount - prvepovitCliqueCount));
                }
                for (daf::Size v = prvePovit; v < i; v++) {
                    newCore.setIfBigger(orderPovit[v],sup[orderPovit[v]].add(prveCore, povitCliqueCount));
                }
    ...
    for (const auto &child: node.children) {
        ...
        if (child->isPivot) {
            povit.push_back(child->v);
            computeSupHelp(*child, k, core, newCore, povit, keepC, sup);
            povit.pop_back();
        } else {
            keepC.push_back(child->v);
            computeSupHelp(*child, k, core, newCore, povit, keepC, sup);
...
std::vector<ThreadSafeMap> computeSup(const MultiBranchTree &tree,
                const daf::CliqueSize k,
                const daf::Size *core,
                MutexStaticVector<daf::Size> &newCore) {
    auto &children = tree.getRoot()->children;
    std::vector<ThreadSafeMap> sup(children.size());
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, children.size()),
        [&](const tbb::blocked_range<size_t>& r) {
            for (size_t i = r.begin(); i != r.end(); ++i) {
                ...
                StaticVector<daf::Size> keepC;
                keepC.push_back(node->v);
                computeSupHelp(*node, k, core, newCore, povitC, keepC, sup);
            }
        });
...
void baseNucleusCoreDecompositionParHash(const MultiBranchTree &tree, daf::CliqueSize k){
    ...
    auto *core = HashPar::countingPerVertex(tree, k);
    auto newCore = MutexStaticVector<daf::Size>(tree.getRoot()->children.size());
    std::cout << "init core: ";
    ...
    while (update) {
        ...
        HashPar::computeSup(tree, k, core, newCore);
        // memcpy(core, newCore.data, tree.getRoot()->children.size() * sizeof(daf::Size));
        // memset(newCore.data, 0, tree.getRoot()->children.size() * sizeof(daf::Size));
        // std::cout << supV << std::endl;
        for (daf::Size i = 0; i < tree.getRoot()->children.size(); i++) {
            if (core[i] > newCore.data[i]) {
                core[i] = newCore.data[i];
                update = true;
            }
            newCore.data[i] = 0;
        }
