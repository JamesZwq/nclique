...
//
// Key optimization over V6: Instead of checking leafAlive[] in the Phase 1
// intersection callback (which still iterates stale entries), we maintain a
// staleCount[v] per vertex. Before intersecting (u,v), if staleCount[u] > 0,
// we purge u's treeGraphV set (erase dead entries), then reset staleCount[u]=0.
// Same for v. This way, the intersection callback never sees stale entries.
//
// Case A: leafAlive[leafId] = 0, increment staleCount[v] for each v on leaf
...
std::vector<std::pair<std::pair<daf::Size, daf::Size>, int> > PlusNucleusEdgeCoreDecompositionSet_ST_V7(
    ...
    auto *countingKE = PlusECDSet_ST_V7::countingPerEdge(tree, edgeGraph, k);
    ...
    daf::StaticVector<PlusECDSet_ST_V7::LeafRmInfo> leafRmInfo(tree.adj_list.size());
    ...
    // V7: staleCount per vertex — tracks how many dead leaf entries remain in treeGraphV[v]
    std::vector<daf::Size> staleCount(treeGraphV.adj_list.size(), 0);
    ...
    // V7: purge lambda — erase dead entries from treeGraphV[v]
    auto purge = [&](daf::Size v) {
        if (staleCount[v] == 0) return;
    ...
    while (remainingInHeap > 0) {
        ...
        // V7: purge stale entries from both vertices BEFORE intersection
        ...
        for (int ei = 0; ei < (int)currentRemoveEdgeIds.size(); ++ei) {
            ...
            daf::intersect_dense_sets(adjEdgeU, adjEdgeV,
                                      [&](const TreeGraphNode &uClique, const TreeGraphNode &vClique) {
                                          ...
                                          // V7: no leafAlive check needed — stale entries already purged
        ...
        caseBLeafIds.clear();
        {
            ...
            for (int leafIdIdx = 0; leafIdIdx < (int)removedLeaf.size(); ++leafIdIdx) {
                ...
                PlusECDSet_ST_V7::LeafRmInfo &leafRm = leafRmInfo[leafId];
                ...
                if (isDeadLeaf) {
                    ...
                    // V7: skip removeNbr — mark dead + increment staleCount
                    leafAlive[leafId] = 0;
                    for (const auto& node : leaf)
                        staleCount[node.v]++;
        ...
        for (int bi = 0; bi < (int)caseBLeafIds.size(); ++bi) {
            ...
            bkRmEdge::bronKerbosch(leafRef, leafRm.removedEdges, k,
                [&](const bkRmEdge::Bitset &c, const bkRmEdge::Bitset &pivots) {
                    ...
                    // Grow leafAlive and staleCount for new sub-leaves
                    if (newId >= leafAlive.size()) {
                        leafAlive.resize(newId * 1.5 + 1, 0);
                        staleCount.resize(newId * 1.5 + 1, 0);
                    }
                    leafAlive[newId] = 1;
                    ...
                    // Grow staleCount for vertices (treeGraphV may have grown)
                    if (treeGraphV.adj_list.size() > staleCount.size())
                        staleCount.resize(treeGraphV.adj_list.size(), 0);
                    daf::Size np = k - newKeepC.size();
                    double KtoK = 0, KtoP = 0, PtoP = 0;
                    if (np <= newPivot.size() && newKeepC.size() > 1) {
                        KtoK = std::llround(nCr[newPivot.size()][np]);
                        PlusECDSet_ST_V7::processEdgePairs(newKeepC, KtoK, addW);
                    }
                    int nPP = int(np) - 2;
                    if (0 <= nPP && nPP <= int(newPivot.size()) - 2) {
                        PtoP = std::llround(nCr[newPivot.size() - 2][nPP]);
                        PlusECDSet_ST_V7::processEdgePairs(newPivot, PtoP, addW);
                    }
                    int nKP = int(np) - 1;
                    if (0 <= nKP && nKP <= int(newPivot.size()) - 1) {
                        KtoP = std::llround(nCr[newPivot.size() - 1][nKP]);
                        PlusECDSet_ST_V7::processEdgePairs(newKeepC, newPivot, KtoP, addW);
                    }
                    newPivot.clear(); newKeepC.clear();
            ...
            if (needPivot <= povit.size()) {
                ...
                PlusECDSet_ST_V7::processEdgePairs(keepC, KtoK, removeW);
            }
            int needPP = int(needPivot) - 2;
            if (0 <= needPP && needPP <= int(povit.size()) - 2) {
                PtoP = std::llround(nCr[povit.size() - 2][needPP]);
                PlusECDSet_ST_V7::processEdgePairs(povit, PtoP, removeW);
            }
            int needKP = int(needPivot) - 1;
            if (0 <= needKP && needKP <= int(povit.size()) - 1) {
                KtoP = std::llround(nCr[povit.size() - 1][needKP]);
                PlusECDSet_ST_V7::processEdgePairs(keepC, povit, KtoP, removeW);
            }
