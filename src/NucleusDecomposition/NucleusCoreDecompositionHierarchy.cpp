...
std::vector<std::pair<std::pair<daf::Size, daf::Size>, int> > NucleusCoreDecompositionHierarchy(
    ...
    if (r < 3 || r >= s) {
        throw std::invalid_argument("Invalid clique size r or s: r must be >= 3 and r > s.");
    }
    ...
    auto [countingKE, countingRClique] = daf::timeCount("countingPerEdgeAndRClique",
                                                        [&]() {
                                                            return CDSetH::countingPerEdgeAndRClique(
                                                                tree, cliqueIndex, edgeGraph, r, s);
                                                        });

    std::vector<daf::Size> coreRClique(countingRClique.size(), std::numeric_limits<daf::Size>::max());

    // BipartiteDSU disJoinSet(countingRClique.size(), tree.adj_list.size());


    auto *coreE = new double[edgeGraph.adj_list.size()];
    memset(coreE, 0, edgeGraph.adj_list.size() * sizeof(double));
    // std::vector<double> leafCore = CDSetH::initLeafCore(tree, countingKE, k, edgeGraph);
#ifndef NDEBUG
    tree.printGraphPerV();
    // daf::printArray(countingKE, edgeGraph.adj_list.size());
    CDSetH::printEdgeCore(edgeGraph, countingKE);
    // CDSetH::printEdgeCore(edgeGraph, degreeE);
#endif
    ...
    daf::StaticVector<CDSetH::LeafRmInfo> leafRmInfo(tree.adj_list.size());
    leafRmInfo.c_size = tree.adj_list.size();

    CDSetH::DHeap heap{CDSetH::CompareRClique(countingRClique.data())};
    std::vector<CDSetH::DHeap::handle_type> heapHandles(cliqueIndex.size());
...
#ifndef NDEBUG
    std::cout << "coreE: ";
    CDSetH::printEdgeCore(edgeGraph, coreE);
    std::cout << "countingKE: ";
    CDSetH::printEdgeCore(edgeGraph, countingKE);
    ...
    double minCore = 0;
    int numProgress = 0;
    // std::vector<std::pair<int, BipartiteDSU>> cliqueDisJoinSet;
    // cliqueDisJoinSet.reserve(1000);
    std::vector<std::tuple<int, daf::Size, daf::Size> > unionRecord;
    unionRecord.reserve(tree.adj_list.size() * 10);
    int numUnion = 0;
    while (!heap.empty()) {
        ...
        // if (cliqueDisJoinSet.empty() || cliqueDisJoinSet.back().first != minCore) {
        //     cliqueDisJoinSet.emplace_back(minCore, BipartiteDSU(countingRClique.size(), tree.adj_list.size()));
        // }

        // auto &currentCliqueDisJoinSet = cliqueDisJoinSet.back().second;
        ...
        for (auto leafIdIdx = 0; leafIdIdx < removedLeaf.size(); ++leafIdIdx) {
            ...
            CDSetH::LeafRmInfo &leafRm = leafRmInfo[leafId];
            if (leaf.empty()) {
            ...
            if (leafRm.removedKeepC || needPivot > povit.size() - leafRm.removedPivots.size()) {
                daf::enumerateCombinations(leaf, r, [&](const daf::StaticVector<TreeGraphNode> &newLeaf) {
                    ...
                    if (coreRClique[cliqueIndexId] >= minCore) {
                        unionRecord.emplace_back(minCore, cliqueIndexId, leafId);
                        numUnion++;
                    }
                ...
                daf::enumerateCombinations(oldLeaf, r, [&](const daf::StaticVector<TreeGraphNode> &newLeaf) {
                    ...
                    if (coreRClique[cliqueIndexId] >= minCore) {
                        // currentCliqueDisJoinSet.uniteAB(cliqueIndexId, leafId);
                        unionRecord.emplace_back(minCore, cliqueIndexId, leafId);
                        numUnion++;
                    }
                ...
                if (!leafRm.removedPivots.empty() && needPivot <= povit.size() - leafRm.removedPivots.size()) {
                    ...
                    daf::enumAtLeastOneFromTwo(leafRm.removedPivots, tree.adj_list[leafId], r,
                                               [&](daf::Size *pivots, size_t pCount, TreeGraphNode *cliqueList,
                                                   ...
                                                   const auto cliqueIndexId = cliqueIndex.byClique(
    ...
    if (numCounting != 0) {
        ...
        std::cout << "numCounting: " << numCounting << std::endl;
    }
    ...
    newPivot.free();
    // leafIds.free();
    std::cout << "numUnion: " << numUnion << std::endl;
    return sortedK;
}
