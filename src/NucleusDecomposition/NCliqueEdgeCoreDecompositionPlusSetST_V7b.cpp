...
//
// Key optimization over V6: Instead of checking leafAlive[] on every
// intersection callback hit (which still iterates stale entries), we track
// which vertices have stale entries (dirty set) and periodically purge them
// every COMPACT_INTERVAL peeling iterations. Between compactions, the
// leafAlive check in the callback filters stale entries (like V6).
//
// Case A: leafAlive[leafId] = 0, mark leaf's vertices dirty
// Case C (dead): same as Case A
// Case C (surviving): still physically erases removed pivots from treeGraphV
// Case B: still physically erases all entries before recycling (safe)
//
...
std::vector<std::pair<std::pair<daf::Size, daf::Size>, int> > PlusNucleusEdgeCoreDecompositionSet_ST_V7b(
    ...
    auto *countingKE = PlusECDSet_ST_V7b::countingPerEdge(tree, edgeGraph, k);
    ...
    daf::StaticVector<PlusECDSet_ST_V7b::LeafRmInfo> leafRmInfo(tree.adj_list.size());
    leafRmInfo.c_size = tree.adj_list.size();

    // leafAlive bitset: 1=live, 0=dead
    std::vector<uint8_t> leafAlive(tree.adj_list.size(), 0);
    for (daf::Size i = 0; i < tree.adj_list.size(); ++i)
        if (tree.adj_list[i].size() >= k) leafAlive[i] = 1;

    // V7b: dirty vertex tracking for periodic compaction
    std::vector<uint8_t> vertexDirty(treeGraphV.adj_list.size(), 0);
    std::vector<daf::Size> dirtyVertices;
    dirtyVertices.reserve(4096);
    int itersSinceCompact = 0;
    const int COMPACT_INTERVAL = 100;
    ...
    while (remainingInHeap > 0) {
        ...
        // V7b: leafAlive check in callback (entries may be stale between compactions)
        ...
        for (int ei = 0; ei < (int)currentRemoveEdgeIds.size(); ++ei) {
            ...
            daf::intersect_dense_sets(adjEdgeU, adjEdgeV,
                                      [&](const TreeGraphNode &uClique, const TreeGraphNode &vClique) {
                                          ...
                                          if (!leafAlive[leafId]) return;  // V7b: needed between compactions
        ...
        caseBLeafIds.clear();
        {
            ...
            for (int leafIdIdx = 0; leafIdIdx < (int)removedLeaf.size(); ++leafIdIdx) {
                ...
                PlusECDSet_ST_V7b::LeafRmInfo &leafRm = leafRmInfo[leafId];
                ...
                if (isDeadLeaf) {
                    ...
                    // V7b: skip removeNbr — mark dead + track dirty vertices
                    leafAlive[leafId] = 0;
                    for (const auto& node : leaf) {
                        if (!vertexDirty[node.v]) {
                            vertexDirty[node.v] = 1;
                    ...
                    if (!leafRm.removedPivots.empty() && needPivot <= tPovit.size() - leafRm.removedPivots.size()) {
                        ...
                        newLeafF.free();
                        // Case C surviving: still physically erase removed pivots from treeGraphV
                        ...
                        // Case C dead (pivot exhaustion): leaf dies
        ...
        // V7b: Periodic compaction — purge dirty vertices every COMPACT_INTERVAL iterations
        itersSinceCompact++;
        if (itersSinceCompact >= COMPACT_INTERVAL && !dirtyVertices.empty()) {
            for (auto v : dirtyVertices) {
                auto &set = treeGraphV.adj_list[v];
                for (auto it = set.begin(); it != set.end(); ) {
                    if (!leafAlive[it->v]) it = set.erase(it);
                    else ++it;
                }
                vertexDirty[v] = 0;
            }
        ...
        for (int bi = 0; bi < (int)caseBLeafIds.size(); ++bi) {
            ...
            // Case B: Remove old leaf from treeGraphV — physical erase (safe to recycle after)
            ...
            bkRmEdge::bronKerbosch(leafRef, leafRm.removedEdges, k,
                [&](const bkRmEdge::Bitset &c, const bkRmEdge::Bitset &pivots) {
                    ...
                    // Grow leafAlive for new sub-leaves
                    if (newId >= leafAlive.size()) {
                        leafAlive.resize(newId * 1.5 + 1, 0);
                        vertexDirty.resize(std::max(vertexDirty.size(), (size_t)(newId * 1.5 + 1)), 0);
                    }
                    leafAlive[newId] = 1;
                    ...
                    // Grow vertexDirty if treeGraphV grew
                    if (treeGraphV.adj_list.size() > vertexDirty.size())
                        vertexDirty.resize(treeGraphV.adj_list.size(), 0);
                    daf::Size np = k - newKeepC.size();
                    double KtoK = 0, KtoP = 0, PtoP = 0;
                    if (np <= newPivot.size() && newKeepC.size() > 1) {
                        KtoK = std::llround(nCr[newPivot.size()][np]);
                        PlusECDSet_ST_V7b::processEdgePairs(newKeepC, KtoK, addW);
                    }
                    int nPP = int(np) - 2;
                    if (0 <= nPP && nPP <= int(newPivot.size()) - 2) {
                        PtoP = std::llround(nCr[newPivot.size() - 2][nPP]);
                        PlusECDSet_ST_V7b::processEdgePairs(newPivot, PtoP, addW);
                    }
                    int nKP = int(np) - 1;
                    if (0 <= nKP && nKP <= int(newPivot.size()) - 1) {
                        KtoP = std::llround(nCr[newPivot.size() - 1][nKP]);
                        PlusECDSet_ST_V7b::processEdgePairs(newKeepC, newPivot, KtoP, addW);
                    }
                    newPivot.clear(); newKeepC.clear();
            ...
            if (needPivot <= povit.size()) {
                ...
                PlusECDSet_ST_V7b::processEdgePairs(keepC, KtoK, removeW);
            }
            int needPP = int(needPivot) - 2;
            if (0 <= needPP && needPP <= int(povit.size()) - 2) {
                PtoP = std::llround(nCr[povit.size() - 2][needPP]);
                PlusECDSet_ST_V7b::processEdgePairs(povit, PtoP, removeW);
            }
            int needKP = int(needPivot) - 1;
            if (0 <= needKP && needKP <= int(povit.size()) - 1) {
                KtoP = std::llround(nCr[povit.size() - 1][needKP]);
                PlusECDSet_ST_V7b::processEdgePairs(keepC, povit, KtoP, removeW);
            }

            // Case B: safe to recycle — all entries physically erased above
