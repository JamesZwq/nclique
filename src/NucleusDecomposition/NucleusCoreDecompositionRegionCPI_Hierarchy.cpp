...
// Simple DSU for hierarchy construction
struct SimpleDSU {
    std::vector<int> parent, rank_;
    int numComponents;
    SimpleDSU(int n) : parent(n), rank_(n, 0), numComponents(n) {
...
NucleusCoreDecompositionRClique_RegionCPI_Hierarchy(
    ...
    // Fused fully-mergeable check (see RegionCPI base + docs/region_cpi_theorems.md §7).
    ...
    // Save fully-mergeable MCs for hierarchy (before they're removed)
    struct MergedMC { std::vector<daf::Size> verts; double coreVal; };
    std::vector<MergedMC> mergedMCs;
    for (daf::Size rid = 0; rid < numRegions; ++rid) {
        if (!fullyMergeable[rid]) continue;
        int mSize = (int)regionVerts[rid].size();
        double cv = (mSize >= (int)s) ? nCr[mSize - r][s - r] : 0.0;
        if (cv > 0) mergedMCs.push_back({regionVerts[rid], cv});
    }
    ...
    // --- Hierarchy: vertex-level connected components at each core level ---
    // Compute per-vertex core value = max core over all containing r-cliques.
    // Phase 1 (private cloud): vertex core ≥ MC's core value
    // Phase 2 (active tuples): vertex core = max of tuple core from peeling
    //
    // We'll record tuple core values during peeling, then post-process.

    // Per-tuple core value (filled during peeling)
    std::vector<double> tupleCorePeeled(rTuples.size(), 0.0);

    // Build class → vertices mapping for fast lookup
    std::vector<std::vector<daf::Size>> classVerts(numClasses);
    for (daf::Size v = 0; v < numVertices; ++v)
        if (classOf[v] != INVALID) classVerts[classOf[v]].push_back(v);

    // Per-vertex core: every vertex gets max C(|MC|-r, s-r) over all MCs containing it
    std::vector<double> vertexCore(numVertices, 0.0);
    // From Step 1b fully-mergeable MCs
    for (auto &mc : mergedMCs)
        for (auto v : mc.verts)
            if (v < numVertices) vertexCore[v] = std::max(vertexCore[v], mc.coreVal);
    // From remaining (non-mergeable) MCs
    for (daf::Size rid = 0; rid < numRegions; ++rid) {
        double cv = mcCoreVal[rid];
        for (auto v : regionVerts[rid])
            if (v < numVertices) vertexCore[v] = std::max(vertexCore[v], cv);
    }
    ...
    while (numPeeled < rTuples.size()) {
        ...
        for (auto idx : batch) {
            ...
            tupleCorePeeled[idx] = (double)coreLevel;
        }
    ...
    // --- Post-peeling: hierarchy via tuple-level s-connectivity ---
    //
    // Two alive tuples that both cover the same original CPI path are s-connected
    // (Thm: CPI-path connectivity). All realizations of one tuple are mutually
    // s-connected through any M in Pi(tau) (Thm: intra-tuple s-connectivity),
    // so one DSU node per tuple suffices. Region-private vertices merge into
    // the component containing any non-private class of the same region; by the
    // minCore floor (Thm 7.2), such a tuple is always already active when the
    // region's private block is activated at level mcCoreVal(M). Fully-mergeable
    // regions form isolated components.
    //
    // Incremental algorithm: process events in DESC order of core level. A
    // single DSU accumulates unions; component structure at level k is the
    // snapshot after processing all events with level >= k.
    auto tHierStart = std::chrono::high_resolution_clock::now();

    enum HierEvType { EV_TUPLE = 0, EV_PRIVATE = 1, EV_FM = 2 };
    struct HierEv { double level; int type; daf::Size idx; };
    std::vector<HierEv> events;
    events.reserve(rTuples.size() + numRegions + mergedMCs.size());
    for (daf::Size tidx = 0; tidx < rTuples.size(); ++tidx)
        if (tupleCorePeeled[tidx] > 0)
            events.push_back({tupleCorePeeled[tidx], EV_TUPLE, tidx});
    if (enablePrivateCloud) {
        for (daf::Size rid = 0; rid < numRegions; ++rid) {
            if (mcCoreVal[rid] <= 0) continue;
            bool hasPriv = false;
            for (auto cid : classesInRegion[rid])
                if (isPrivateClass[cid]) { hasPriv = true; break; }
            if (hasPriv) events.push_back({mcCoreVal[rid], EV_PRIVATE, rid});
        }
    }
    for (daf::Size mi = 0; mi < mergedMCs.size(); ++mi)
        if (mergedMCs[mi].coreVal > 0)
            events.push_back({mergedMCs[mi].coreVal, EV_FM, mi});
    std::sort(events.begin(), events.end(), [](const HierEv &a, const HierEv &b) {
        if (a.level != b.level) return a.level > b.level;
        return a.type < b.type;
    });

    // DSU over [tuples | regions (private block) | fully-merged MCs].
    const daf::Size nodeBase_region = rTuples.size();
    const daf::Size nodeBase_fm     = rTuples.size() + numRegions;
    const daf::Size totalNodes      = rTuples.size() + numRegions + mergedMCs.size();
    ...
    std::vector<int>  pathRep(pathInfos.size(), -1);
    std::vector<int>  classOwner(numClasses, -1); // first tuple node seen with this class

    // For each non-private class c, the regions that have c as a non-private class
    // (= pi(c) for non-private c). Used for reverse-direction region-private merge
    // when a tuple claims c AFTER the region's EV_PRIVATE has already fired.
    std::vector<std::vector<daf::Size>> classNonPrivRegions(numClasses);
    for (daf::Size rid = 0; rid < numRegions; ++rid) {
        for (auto cid : classesInRegion[rid])
            if (!isPrivateClass[cid]) classNonPrivRegions[cid].push_back(rid);
    }

    // Sorted class IDs per DSU root — dedupes the vertex count within a component
    // by tracking which classes have been added (classes are pairwise disjoint).
    // FM nodes leave this empty (they add their vertex count directly without classes).
    std::vector<std::vector<daf::Size>> rootClasses(totalNodes);

    auto addClass = [&](int node, daf::Size cid) {
        int root = dsu.find(node);
        auto &vec = rootClasses[root];
        auto it = std::lower_bound(vec.begin(), vec.end(), cid);
        if (it == vec.end() || *it != cid) {
            vec.insert(it, cid);
            compVertCount[root] += (int)classVerts[cid].size();
        }
    };
    auto mergeRoots = [&](int a, int b) {
        int ra = dsu.find(a), rb = dsu.find(b);
        if (ra == rb) return;
        // Small-to-large on class sets: the bigger becomes the surviving root.
        if (rootClasses[ra].size() < rootClasses[rb].size()) std::swap(ra, rb);
        // Merge rb's classes into ra, adding vertex counts only for new classes.
        for (auto cid : rootClasses[rb]) {
            auto &vec = rootClasses[ra];
            auto it = std::lower_bound(vec.begin(), vec.end(), cid);
            if (it == vec.end() || *it != cid) {
                vec.insert(it, cid);
                compVertCount[ra] += (int)classVerts[cid].size();
            }
        }
        // rb's remaining (non-class) vertex contribution — e.g., FM mass — carries over too.
        int rbOther = compVertCount[rb];
        for (auto cid : rootClasses[rb]) rbOther -= (int)classVerts[cid].size();
        if (rbOther > 0) compVertCount[ra] += rbOther;
        rootClasses[rb].clear();
        compVertCount[rb] = 0;
        dsu.unite(ra, rb);
        int newRoot = dsu.find(ra);
        if (newRoot != ra) {
            rootClasses[newRoot] = std::move(rootClasses[ra]);
            compVertCount[newRoot] = compVertCount[ra];
            rootClasses[ra].clear();
            compVertCount[ra] = 0;
        }
    };
    ...
    std::cout << "\n  --- Hierarchy ---" << std::endl;
    size_t levelsSeen = 0;
    std::vector<int> sizes; sizes.reserve(64);
    size_t i = 0;
    while (i < events.size()) {
        double level = events[i].level;
        while (i < events.size() && events[i].level == level) {
            const auto &ev = events[i++];
            if (ev.type == EV_TUPLE) {
                daf::Size tidx = ev.idx;
                int node = (int)tidx;
                active[node] = 1;
                for (auto cid : rTuples[tidx].key) {
                    bool firstClaim = (classOwner[cid] < 0);
                    if (firstClaim) classOwner[cid] = node;
                    addClass(node, cid);
                    // Reverse-direction merge: if a region-private block activated
                    // earlier found no tuple to merge with, it is still isolated.
                    // If this tuple uses a non-private class of that region, merge now.
                    if (firstClaim) {
                        for (auto rid : classNonPrivRegions[cid]) {
                            int rnode = (int)(nodeBase_region + rid);
                            if (active[rnode]) mergeRoots(node, rnode);
                        }
                    }
                }
                for (auto piIdx : tupleToPathInfos[tidx]) {
                    if (pathRep[piIdx] < 0) pathRep[piIdx] = node;
                    else mergeRoots(node, pathRep[piIdx]);
                }
            } else if (ev.type == EV_PRIVATE) {
                daf::Size rid = ev.idx;
                int node = (int)(nodeBase_region + rid);
                active[node] = 1;
                for (auto cid : classesInRegion[rid])
                    if (isPrivateClass[cid]) addClass(node, cid);
                // Merge with the component of any non-private class of M already used by a tuple.
                // If no such tuple has activated yet, the region stays isolated until a later
                // tuple activation does the reverse-direction merge.
                for (auto cid : classesInRegion[rid]) {
                    if (isPrivateClass[cid]) continue;
                    if (classOwner[cid] >= 0) { mergeRoots(node, classOwner[cid]); break; }
                }
            } else { // EV_FM
                daf::Size mi = ev.idx;
        ...
        ++levelsSeen;
        sizes.clear();
        for (int n = 0; n < (int)totalNodes; ++n) {
            if (!active[n]) continue;
            if (dsu.find(n) != n) continue;
            if (compVertCount[n] <= 0) continue;
            sizes.push_back(compVertCount[n]);
        }
        std::sort(sizes.rbegin(), sizes.rend());
        int totalVerts = 0;
        for (auto s : sizes) totalVerts += s;
        std::cout << "  core>=" << std::fixed << std::setprecision(0) << level
                  << "  verts=" << totalVerts << "  comps=" << sizes.size() << "  sizes=[";
        for (size_t j = 0; j < sizes.size() && j < 20; ++j) {
            if (j > 0) std::cout << ",";
            std::cout << sizes[j];
        }
        if (sizes.size() > 20) std::cout << ",...";
        std::cout << "]" << std::endl;
    }
    auto tHierEnd = std::chrono::high_resolution_clock::now();
    auto hierMs = std::chrono::duration_cast<std::chrono::milliseconds>(tHierEnd - tHierStart).count();
    std::cout << "  Hierarchy post-processing: " << hierMs << " ms ("
              << levelsSeen << " levels)" << std::endl;
