...
std::vector<std::pair<std::vector<daf::Size>, int> > NucleusCoreDecompositionCorrect(
    ...
    auto countingRClique = daf::timeCount("countingPerEdgeAndRClique",
                                          [&]() {
                                              return CDSetRSC::countingPerRClique(
    ...
    // CDSetRSC::printEdgeCore(edgeGraph, countingKE);
    ...
    // CDSetRSC::printEdgeCore(edgeGraph, degreeE);
#endif
    ...
    CDSetRSC::DHeap heap{CDSetRSC::CompareRClique(countingRClique.data())};
    heap.reserve(cliqueIndex.size());

    std::vector<CDSetRSC::DHeap::handle_type> heapHandles(cliqueIndex.size());
