...
//
// Instead of bucket array + std::set overflow, use a sorted map of buckets.
// Each distinct support value maps to a vector of clique IDs.
// ExtractMin: O(1) via map.begin(). DecreaseKey: O(log B) where B = distinct values.
// No overflow, no bucket threshold, handles any support value.
//
...
std::vector<std::pair<std::vector<daf::Size>, double>> NucleusCoreDecompositionRClique_ST_V20(
    ...
    // ========== Sparse Bucket PQ (V20) ==========
    // map<double, vector<Size>>: each distinct support value → clique IDs
    std::map<double, std::vector<daf::Size>> spBuckets;
    std::vector<double> spVal(nClique);          // current bucket value per clique
    std::vector<daf::Size> spPos(nClique);       // position in its bucket vector

    for (daf::Size i = 0; i < nClique; ++i) {
        double v = countingRClique[i];
        spVal[i] = v;
        auto &bk = spBuckets[v];
        spPos[i] = bk.size();
        bk.push_back(i);
    ...
    printf("Sparse bucket PQ: %zu distinct values, %u cliques\n", spBuckets.size(), nClique);

    auto bucketMove = [&](daf::Size id) {
        if (!rCliqueInHeap[id]) return;
        double newVal = std::max(0.0, countingRClique[id]);
        double oldVal = spVal[id];
        if (newVal == oldVal) return;

        // Remove from old bucket (swap-with-last)
        auto it = spBuckets.find(oldVal);
        if (it != spBuckets.end()) {
            auto &vec = it->second;
            daf::Size pos = spPos[id];
            if (pos < vec.size() - 1) {
                daf::Size last = vec.back();
                vec[pos] = last;
                spPos[last] = pos;
            }
            vec.pop_back();
            if (vec.empty()) spBuckets.erase(it);
        }

        // Insert into new bucket
        spVal[id] = newVal;
        auto &newBk = spBuckets[newVal];
        spPos[id] = newBk.size();
        newBk.push_back(id);
    };
    ...
    while (remainingInHeap > 0) {
        ...
        // --- Pop (sparse bucket) ---
        // Skip empty / stale buckets at the front
        while (!spBuckets.empty()) {
            auto it = spBuckets.begin();
            // Remove stale entries
            auto &vec = it->second;
            while (!vec.empty() && !rCliqueInHeap[vec.back()]) vec.pop_back();
            if (vec.empty()) { spBuckets.erase(it); continue; }
            break;
        }
        if (spBuckets.empty()) break;

        auto it = spBuckets.begin();
        minCore = std::max(it->first, minCore);

        // Pop all cliques at minCore level (and any ≤ minCore)
        while (!spBuckets.empty() && spBuckets.begin()->first <= minCore) {
            auto bit = spBuckets.begin();
            auto &vec = bit->second;
            while (!vec.empty()) {
                auto id = vec.back(); vec.pop_back();
