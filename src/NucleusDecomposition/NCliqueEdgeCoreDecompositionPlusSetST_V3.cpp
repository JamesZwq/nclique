...
std::vector<std::pair<std::pair<daf::Size, daf::Size>, int>> PlusNucleusEdgeCoreDecompositionSet_ST_V3(
    ...
    auto *countingKE = R2STV3::countingPerEdge(tree, edgeGraph, k);
    ...
    daf::StaticVector<R2STV3::LeafRmInfo> leafRmInfo(tree.adj_list.size());
    ...
    std::cout << "=========================begin (R2 ST V3)===========================" << std::endl;
    ...
    while (remainingInHeap > 0) {
        ...
        caseBLeafIds.clear();
        {
            ...
            for (int leafIdIdx = 0; leafIdIdx < (int)removedLeaf.size(); ++leafIdIdx) {
                ...
                R2STV3::LeafRmInfo &leafRm = leafRmInfo[leafId];
        ...
        for (int bi = 0; bi < (int)caseBLeafIds.size(); ++bi) {
            ...
            bkRmEdge::bronKerbosch(leafRef, leafRm.removedEdges, k,
                [&](const bkRmEdge::Bitset &c, const bkRmEdge::Bitset &pivots) {
                    ...
                    if (np <= newPivot.size() && newKeepC.size() > 1) { double w = std::llround(nCr[newPivot.size()][np]); R2STV3::processEdgePairs(newKeepC, w, addW); }
                    int nPP = int(np) - 2;
                    if (0 <= nPP && nPP <= int(newPivot.size()) - 2) { double w = std::llround(nCr[newPivot.size() - 2][nPP]); R2STV3::processEdgePairs(newPivot, w, addW); }
                    int nKP = int(np) - 1;
                    if (0 <= nKP && nKP <= int(newPivot.size()) - 1) { double w = std::llround(nCr[newPivot.size() - 1][nKP]); R2STV3::processEdgePairs(newKeepC, newPivot, w, addW); }
                    newPivot.clear(); newKeepC.clear();
            ...
            if (needPivot <= povit.size()) { KtoK = std::llround(nCr[povit.size()][needPivot]); R2STV3::processEdgePairs(keepC, KtoK, removeW); }
            int needPP = int(needPivot) - 2;
            if (0 <= needPP && needPP <= int(povit.size()) - 2) { PtoP = std::llround(nCr[povit.size() - 2][needPP]); R2STV3::processEdgePairs(povit, PtoP, removeW); }
            int needKP = int(needPivot) - 1;
            if (0 <= needKP && needKP <= int(povit.size()) - 1) { KtoP = std::llround(nCr[povit.size() - 1][needKP]); R2STV3::processEdgePairs(keepC, povit, KtoP, removeW); }
