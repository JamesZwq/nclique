//
// R2 ST V6: Case A Skip Mutation via leafAlive Bitset
//
// Key optimization: Instead of physically removing dead leaf entries from
// treeGraphV hash sets (expensive removeNbr calls), we check a leafAlive[]
// bitset inside the Phase 1 intersection callback to skip dead leaves (O(1)).
//
// Case A: leafAlive[leafId] = 0 instead of removeNbr loop + recycleNode
// Case C (surviving): still physically erases removed pivots from treeGraphV
// Case C (dead): leafAlive[leafId] = 0 instead of removeNbr loop + recycleNode
...
std::vector<std::pair<std::pair<daf::Size, daf::Size>, int> > PlusNucleusEdgeCoreDecompositionSet_ST_V6(
    ...
    auto *countingKE = PlusECDSet_ST_V6::countingPerEdge(tree, edgeGraph, k);
    ...
    daf::StaticVector<PlusECDSet_ST_V6::LeafRmInfo> leafRmInfo(tree.adj_list.size());
    leafRmInfo.c_size = tree.adj_list.size();

    // leafAlive bitset: 1=live, 0=dead. Case A sets to 0 instead of calling removeNbr.
    ...
    while (remainingInHeap > 0) {
        ...
        for (int ei = 0; ei < (int)currentRemoveEdgeIds.size(); ++ei) {
            ...
            daf::intersect_dense_sets(adjEdgeU, adjEdgeV,
                                      [&](const TreeGraphNode &uClique, const TreeGraphNode &vClique) {
                                          ...
                                          if (!leafAlive[leafId]) return;  // skip dead leaves
        ...
        caseBLeafIds.clear();
        {
            ...
            for (int leafIdIdx = 0; leafIdIdx < (int)removedLeaf.size(); ++leafIdIdx) {
                ...
                PlusECDSet_ST_V6::LeafRmInfo &leafRm = leafRmInfo[leafId];
                ...
                if (isDeadLeaf) {
                    ...
                    // V6: skip removeNbr — mark dead in leafAlive instead
                    leafAlive[leafId] = 0;
                    tree.adj_list[leafId].clear();
                    // Do NOT recycle — stale entries in treeGraphV would conflict with reused ID
                } else {
        ...
        for (int bi = 0; bi < (int)caseBLeafIds.size(); ++bi) {
            ...
            bkRmEdge::bronKerbosch(leafRef, leafRm.removedEdges, k,
                [&](const bkRmEdge::Bitset &c, const bkRmEdge::Bitset &pivots) {
                    ...
                    if (np <= newPivot.size() && newKeepC.size() > 1) {
                        ...
                        PlusECDSet_ST_V6::processEdgePairs(newKeepC, KtoK, addW);
                    }
                    int nPP = int(np) - 2;
                    if (0 <= nPP && nPP <= int(newPivot.size()) - 2) {
                        PtoP = std::llround(nCr[newPivot.size() - 2][nPP]);
                        PlusECDSet_ST_V6::processEdgePairs(newPivot, PtoP, addW);
                    }
                    int nKP = int(np) - 1;
                    if (0 <= nKP && nKP <= int(newPivot.size()) - 1) {
                        KtoP = std::llround(nCr[newPivot.size() - 1][nKP]);
                        PlusECDSet_ST_V6::processEdgePairs(newKeepC, newPivot, KtoP, addW);
                    }
                    newPivot.clear(); newKeepC.clear();
            ...
            if (needPivot <= povit.size()) {
                ...
                PlusECDSet_ST_V6::processEdgePairs(keepC, KtoK, removeW);
            }
            int needPP = int(needPivot) - 2;
            if (0 <= needPP && needPP <= int(povit.size()) - 2) {
                PtoP = std::llround(nCr[povit.size() - 2][needPP]);
                PlusECDSet_ST_V6::processEdgePairs(povit, PtoP, removeW);
            }
            int needKP = int(needPivot) - 1;
            if (0 <= needKP && needKP <= int(povit.size()) - 1) {
                KtoP = std::llround(nCr[povit.size() - 1][needKP]);
                PlusECDSet_ST_V6::processEdgePairs(keepC, povit, KtoP, removeW);
            }
