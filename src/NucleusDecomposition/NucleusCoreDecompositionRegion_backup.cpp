...
NucleusCoreDecompositionRClique_Region(
    ...
    while (numPeeled < tuples.size()) {
        ...
        peeled[idx] = true;
        numPeeled++;
        coreDist[currentLevel] += tuples[idx].mult;
    ...
    int shown = 0;
    for (auto it = coreDist.rbegin(); it != coreDist.rend() && shown < 15; ++it, ++shown)
        std::cout << "    core=" << it->first << " count=" << it->second << std::endl;
    if (coreDist.size() > 15)
        std::cout << "    ... (" << coreDist.size() << " distinct core values)" << std::endl;
