...
//
// Simplest stale entry mitigation: before each Phase 1 intersection of (u,v),
// always iterate both treeGraphV sets and erase dead entries. No staleCount,
// no dirty tracking. Guaranteed clean sets before every intersection, but
// may over-purge clean vertices.
//
// Case A: leafAlive[leafId] = 0 (no removeNbr, no staleCount)
...
std::vector<std::pair<std::pair<daf::Size, daf::Size>, int> > PlusNucleusEdgeCoreDecompositionSet_ST_V7c(
    ...
    auto *countingKE = PlusECDSet_ST_V7c::countingPerEdge(tree, edgeGraph, k);
    ...
    daf::StaticVector<PlusECDSet_ST_V7c::LeafRmInfo> leafRmInfo(tree.adj_list.size());
    ...
    // V7c: always-purge lambda — erase all dead entries from treeGraphV[v]
    ...
    while (remainingInHeap > 0) {
        ...
        // V7c: always purge both vertices before intersection
        ...
        for (int ei = 0; ei < (int)currentRemoveEdgeIds.size(); ++ei) {
            ...
            daf::intersect_dense_sets(adjEdgeU, adjEdgeV,
                                      [&](const TreeGraphNode &uClique, const TreeGraphNode &vClique) {
                                          ...
                                          // V7c: no leafAlive check needed — stale entries already purged
        ...
        caseBLeafIds.clear();
        {
            ...
            for (int leafIdIdx = 0; leafIdIdx < (int)removedLeaf.size(); ++leafIdIdx) {
                ...
                PlusECDSet_ST_V7c::LeafRmInfo &leafRm = leafRmInfo[leafId];
                ...
                if (isDeadLeaf) {
                    ...
                    // V7c: skip removeNbr — just mark dead
                    leafAlive[leafId] = 0;
        ...
        for (int bi = 0; bi < (int)caseBLeafIds.size(); ++bi) {
            ...
            bkRmEdge::bronKerbosch(leafRef, leafRm.removedEdges, k,
                [&](const bkRmEdge::Bitset &c, const bkRmEdge::Bitset &pivots) {
                    ...
                    if (newId >= leafAlive.size()) leafAlive.resize(newId * 1.5 + 1, 0);
                    leafAlive[newId] = 1;
                    ...
                    if (np <= newPivot.size() && newKeepC.size() > 1) {
                        ...
                        PlusECDSet_ST_V7c::processEdgePairs(newKeepC, KtoK, addW);
                    }
                    int nPP = int(np) - 2;
                    if (0 <= nPP && nPP <= int(newPivot.size()) - 2) {
                        PtoP = std::llround(nCr[newPivot.size() - 2][nPP]);
                        PlusECDSet_ST_V7c::processEdgePairs(newPivot, PtoP, addW);
                    }
                    int nKP = int(np) - 1;
                    if (0 <= nKP && nKP <= int(newPivot.size()) - 1) {
                        KtoP = std::llround(nCr[newPivot.size() - 1][nKP]);
                        PlusECDSet_ST_V7c::processEdgePairs(newKeepC, newPivot, KtoP, addW);
                    }
                    newPivot.clear(); newKeepC.clear();
            ...
            if (needPivot <= povit.size()) {
                ...
                PlusECDSet_ST_V7c::processEdgePairs(keepC, KtoK, removeW);
            }
            int needPP = int(needPivot) - 2;
            if (0 <= needPP && needPP <= int(povit.size()) - 2) {
                PtoP = std::llround(nCr[povit.size() - 2][needPP]);
                PlusECDSet_ST_V7c::processEdgePairs(povit, PtoP, removeW);
            }
            int needKP = int(needPivot) - 1;
            if (0 <= needKP && needKP <= int(povit.size()) - 1) {
                KtoP = std::llround(nCr[povit.size() - 1][needKP]);
                PlusECDSet_ST_V7c::processEdgePairs(keepC, povit, KtoP, removeW);
            }
