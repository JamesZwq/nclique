...
//
// Key innovation: instead of BK re-enumeration to split leaves, maintain
// a per-leaf "exception list" of removed r-cliques. Each new removal computes
// the incremental delta via inclusion-exclusion over the exception list.
//
// incrementalDelta(Q, R_new, {E1,...,Ek}) =
//   Σ_{S⊆{E1,...,Ek}} (-1)^|S| × C(P - |Q∪R∪∪S|_pivots, η - |Q∪R∪∪S|_pivots)
//
// Uses ORIGINAL leaf parameters (P, η) — always correct because we're computing
// subsets of the original s-clique space. No tree splitting, no new nodes.
//
// Falls back to LeafDeath when exceptions exceed MAX_EXCEPTIONS (bounded termination).
//
// Complexity: O(2^k × |leafCliqueInfo|) per removal where k = exception count
// vs BK: O(r^d) with tree allocation overhead
//
...
std::vector<std::pair<std::vector<daf::Size>, double>> NucleusCoreDecompositionRClique_ST_V15(
    ...
    std::vector<std::vector<RCliqueSTv15::LeafCliqueEntry>> leafCliqueInfo(numLeaves);
    std::vector<std::vector<daf::Size>> cliqueLeafIds;
    ...
    if (!prebuiltIndex) {
        ...
        daf::timeCount("fused build+dualIndex (ST_V11)", [&]() {
            cliqueIndex.buildWithFullEnum(tree, maxV,
                [&](daf::Size leafIdx, StaticCliqueIndex::Id cliqueId, daf::CliqueSize subNumPivot,
                    ...
                    RCliqueSTv15::LeafCliqueEntry entry;
        ...
        daf::timeCount("dualIndex only (prebuilt CI)", [&]() {
            auto dualIdx = RCliqueSTv15::buildDualIndex(tree, cliqueIndex, r, s);
    ...
    std::vector<RCliqueSTv15::LeafCliqueEntry> reusableEntries;
    ...
    // IET: per-leaf exception list (removed r-clique bitmasks)
    // Each exception is a uint64_t bitmask of pivot positions in the ORIGINAL leaf
    constexpr int MAX_EXCEPTIONS = 15;
    std::vector<std::vector<uint64_t>> leafExceptions(tree.adj_list.size());
    // Per-leaf original parameters (needed for IET delta computation)
    // Stored once, never modified
    std::vector<uint8_t> leafOrigPivotC(tree.adj_list.size(), 0);
    std::vector<int> leafOrigNeedPivot(tree.adj_list.size(), 0);
    std::vector<uint64_t> leafOrigPivotMask(tree.adj_list.size(), 0);
    // Pre-compute
    for (daf::Size L = 0; L < tree.adj_list.size(); ++L) {
        const auto &lf = tree.adj_list[L];
        daf::CliqueSize pC = 0, kC = 0;
        uint64_t pmask = 0;
        for (int i = 0; i < (int)lf.size(); ++i) {
            if (lf[i].isPivot) { pC++; pmask |= (1ULL << i); }
            else kC++;
        }
        leafOrigPivotC[L] = pC;
        leafOrigNeedPivot[L] = s - (int)kC;
        leafOrigPivotMask[L] = pmask;
    }
    // Per-leaf cumulative delta tracking (for correct LeafDeath fallback)
    // leafCumulDelta[leafId][ei] = total IET delta already applied to entry ei
    std::vector<std::vector<long long>> leafCumulDelta(tree.adj_list.size());
    ...
    while (remainingInHeap > 0) {
        ...
        for (daf::Size idx = 0; idx < changedLeaf.size(); ++idx) {
            ...
            // V14 adds analytical sub-leaf construction here for d=1 all-pivot cases.
            // V11 uses BK for all non-LeafDeath cases.
            // V14 adds analytical sub-leaf construction for d=1 all-pivot cases.
            if (false) { // V11: no analytical path
                    ...
                    if (leafId < leafCliqueInfo.size()) {
                        ...
                        for (const auto &entry : leafCliqueInfo[leafId]) {
                            ...
                            if (subP <= np && newPivotC - subP < 1001 && np - subP < 401) {
                                ...
                                RCliqueSTv15::LeafCliqueEntry newEntry;
                ...
                if (leafId < leafCliqueInfo.size()) {
                    const auto &entries = leafCliqueInfo[leafId];
                    const auto &cumulDeltas = (leafId < leafCumulDelta.size()) ? leafCumulDelta[leafId] : std::vector<long long>();
                    bool hasCumul = !cumulDeltas.empty();
                    ...
                    for (size_t ei = 0; ei < entries.size(); ++ei) {
                        ...
                        const auto &entry = entries[ei];
                        if (!rCliqueInHeap[entry.cliqueId]) continue;
                        // Subtract remaining: ncrValue - already-applied IET delta
                        long long remaining = entry.ncrValue - (hasCumul ? cumulDeltas[ei] : 0);
                        if (remaining > 0) {
                            countingRClique[entry.cliqueId] -= remaining;
                            if (countingRClique[entry.cliqueId] < 0) countingRClique[entry.cliqueId] = 0;
                            bucketMove(entry.cliqueId);
                        }
                    }
                ...
                if (leafId < leafCliqueInfo.size())
                    leafCliqueInfo[leafId].clear();
                if (leafId < leafCumulDelta.size())
                    leafCumulDelta[leafId].clear();
                if (leafId < leafExceptions.size())
                    leafExceptions[leafId].clear();
                duration_intersect += std::chrono::duration_cast<std::chrono::nanoseconds>(
                    std::chrono::high_resolution_clock::now() - t_struct).count();
            } else {
                // === V15: Incremental Exception Tracking (IET) ===
                // No BK, no tree splitting. Compute incremental delta via inclusion-exclusion.
                cntBK++; // reuse counter for non-LeafDeath non-ClosedForm cases

                // Check: can we use IET? Need all removed r-cliques to be all-pivot
                // AND exception count must not exceed MAX_EXCEPTIONS
                bool useIET = (forcedPivotRemove == 0);
                if (useIET) {
                    for (auto rmId : removedR) {
                ...
                int currentExceptions = (leafId < leafExceptions.size()) ? (int)leafExceptions[leafId].size() : 0;
                if (useIET && currentExceptions + (int)removedR.size() > MAX_EXCEPTIONS) {
                    useIET = false; // too many exceptions → LeafDeath fallback
                }
                ...
                if (useIET) {
                    ...
                    // Ensure leafExceptions is sized
                    if (leafId >= leafExceptions.size())
                        leafExceptions.resize(leafId + 1);

                    auto &exceptions = leafExceptions[leafId];
                    uint8_t origPivotC = leafOrigPivotC[leafId];
                    int origNeedPivot = leafOrigNeedPivot[leafId];

                    // Ensure cumulative delta vector is sized
                    if (leafId < leafCliqueInfo.size() && leafCumulDelta[leafId].empty())
                        leafCumulDelta[leafId].assign(leafCliqueInfo[leafId].size(), 0);

                    // Process each removed r-clique incrementally
                    for (auto rmId : removedR) {
                        auto rc = cliqueIndex.byId(rmId);
                        uint64_t rmMask = 0;
                        for (auto v : rc) rmMask |= (1ULL << mapRef[v]);

                        if (leafId < leafCliqueInfo.size()) {
                            const auto &entries = leafCliqueInfo[leafId];
                            auto &cumulDeltas = leafCumulDelta[leafId];

                            for (size_t ei = 0; ei < entries.size(); ++ei) {
                                const auto &entry = entries[ei];
                                if (!rCliqueInHeap[entry.cliqueId]) continue;

                                uint64_t qMask = entry.posMask(r);

                                int numEx = (int)exceptions.size();
                                long long delta = 0;

                                for (int mask = 0; mask < (1 << numEx); mask++) {
                                    uint64_t unionMask = qMask | rmMask;
                                    for (int i = 0; i < numEx; i++)
                                        if (mask & (1 << i)) unionMask |= exceptions[i];

                                    int totalPivots = __builtin_popcountll(unionMask & leafOrigPivotMask[leafId]);
                                    int remP = (int)origPivotC - totalPivots;
                                    int remN = origNeedPivot - totalPivots;

                                    long long term = 0;
                                    if (remP >= 0 && remN >= 0 && remP >= remN)
                                        term = (double)(nCr[remP][remN] + 0.5);

                                    int bits = __builtin_popcount(mask);
                                    if (bits % 2 == 0) delta += term;
                                    else delta -= term;
                                }
                                ...
                                if (delta > 0) {
                                    ...
                                    bucketMove(entry.cliqueId);
                                    cumulDeltas[ei] += delta; // track for LeafDeath
                                }
                            }
                        }

                        exceptions.push_back(rmMask);
                    }

                    duration_support += std::chrono::duration_cast<std::chrono::nanoseconds>(
                        std::chrono::high_resolution_clock::now() - t_supp).count();

                    // Leaf survives — no tree mutation
                } else if (leafId < leafExceptions.size() && !leafExceptions[leafId].empty()) {
                    // IET was previously applied but now can't continue →
                    // LeafDeath (subtract remaining = ncrValue - cumulativeDelta)
                    cntLeafDeath++;
                    duration_bk += std::chrono::duration_cast<std::chrono::nanoseconds>(
                        std::chrono::high_resolution_clock::now() - t_bk).count();
                    ...
                    if (leafId < leafCliqueInfo.size()) {
                        const auto &entries = leafCliqueInfo[leafId];
                        const auto &cumulDeltas = leafCumulDelta[leafId];
                        bool hasCumul = !cumulDeltas.empty();
                    ...
                    // No IET history, can't use IET → BK fallback (same as V11)
                    cntBK++;
                    ...
                    duration_support += std::chrono::duration_cast<std::chrono::nanoseconds>(
                        std::chrono::high_resolution_clock::now() - t_supp).count();
                    tree.adj_list[leafId].clear();
                    if (leafId < leafCliqueInfo.size()) leafCliqueInfo[leafId].clear();
                }
            }
        }
    }
