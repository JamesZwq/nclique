//
// R2 ST V8: Case A LeafEdgeInfo Fast-Path
//
// Minimal prototype: only Case A is changed.
// Case C and Case B are identical to ST baseline.
//
// Key optimization: During countingPerEdge init, we also build:
//   leafEdgeInfo[leafId] = flat list of {edgeId, edgeType}
//   leafWKK/leafWPP/leafWKP[leafId] = per-type weights
//
// In Case A (leaf dies, ~82-94% of all cases), instead of:
//   O(|leaf|^2) pairs × getEdgeCompressedId hash lookup per pair
// We do:
//   O(|leafEdgeInfo[leafId]|) linear scan, subtract weight by type
//
// This eliminates all getEdgeCompressedId calls from Case A.
//
...
namespace PlusECDSet_ST_V8 {
    ...
    // Modified countingPerEdge: also builds leafEdgeInfo and per-leaf weights
    struct InitResult {
        double *countingKE;
    ...
    InitResult countingPerEdgeWithIndex(const DynamicGraph<TreeGraphNode> &treeGraph,
        ...
        return {countingE, std::move(leafEdgeInfo), std::move(leafWKK), std::move(leafWPP), std::move(leafWKP)};
    }
}
...
std::vector<std::pair<std::pair<daf::Size, daf::Size>, int> > PlusNucleusEdgeCoreDecompositionSet_ST_V8(
    ...
    // V8: init builds leafEdgeInfo + per-leaf weights alongside countingKE
    auto initResult = PlusECDSet_ST_V8::countingPerEdgeWithIndex(tree, edgeGraph, k);
    ...
    std::cout << "V8 init took: "
              << std::chrono::duration_cast<std::chrono::milliseconds>(time_init - time_start).count()
              << " ms, leafEdgeInfo entries: ";
    size_t totalEntries = 0;
    for (auto &v : leafEdgeInfo) totalEntries += v.size();
    ...
    daf::StaticVector<PlusECDSet_ST_V8::LeafRmInfo> leafRmInfo(tree.adj_list.size());
    ...
    while (remainingInHeap > 0) {
        ...
        caseBLeafIds.clear();
        {
            ...
            for (int leafIdIdx = 0; leafIdIdx < (int)removedLeaf.size(); ++leafIdIdx) {
                ...
                PlusECDSet_ST_V8::LeafRmInfo &leafRm = leafRmInfo[leafId];
                ...
                if (isDeadLeaf) {
                    // ---- Case A: V8 FAST PATH — scan leafEdgeInfo instead of O(|leaf|^2) ----
                    cntA++;

                    // Check if we have leafEdgeInfo for this leaf (init leaves only)
                    if (leafId < leafEdgeInfo.size() && !leafEdgeInfo[leafId].empty()) {
                        // V8 fast path: linear scan of pre-built edge list
                        cntA_fast++;
                        double wKK = leafWKK[leafId];
                        double wPP = leafWPP[leafId];
                        double wKP = leafWKP[leafId];
                        for (auto &entry : leafEdgeInfo[leafId]) {
                            double w;
                            switch (entry.type) {
                                case PlusECDSet_ST_V8::KK: w = wKK; break;
                                case PlusECDSet_ST_V8::PP: w = wPP; break;
                                case PlusECDSet_ST_V8::KP: w = wKP; break;
                            }
                            directSub(entry.edgeId, w);
                        }
                        // Free leafEdgeInfo for this leaf — it's dead, won't be used again
                        leafEdgeInfo[leafId].clear();
                        leafEdgeInfo[leafId].shrink_to_fit();
                    } else {
                        // Fallback: overflow leaves from Case B (no leafEdgeInfo)
                        // Use original O(|leaf|^2) enumeration
                        cntA_fallback++;
                    ...
                    if (!leafRm.removedPivots.empty() && needPivot <= tPovit.size() - leafRm.removedPivots.size()) {
                        ...
                        // V8: invalidate leafEdgeInfo for this leaf (pivots changed, weights stale)
                        ...
                        tree.adj_list[leafId].clear();
                        tree.recycleNode(leafId);
                        // V8: dead leaf, clear leafEdgeInfo
                        if (leafId < leafEdgeInfo.size()) {
                            leafEdgeInfo[leafId].clear();
                            leafEdgeInfo[leafId].shrink_to_fit();
                        }
        ...
        for (int bi = 0; bi < (int)caseBLeafIds.size(); ++bi) {
            ...
            // V8: invalidate leafEdgeInfo for old leaf (it's being split)
            if (leafId < leafEdgeInfo.size()) {
                leafEdgeInfo[leafId].clear();
                leafEdgeInfo[leafId].shrink_to_fit();
            }
            ...
            bkRmEdge::bronKerbosch(leafRef, leafRm.removedEdges, k,
                [&](const bkRmEdge::Bitset &c, const bkRmEdge::Bitset &pivots) {
                    ...
                    if (np <= newPivot.size() && newKeepC.size() > 1) {
                        ...
                        PlusECDSet_ST_V8::processEdgePairs(newKeepC, KtoK, addW);
                    }
                    int nPP = int(np) - 2;
                    if (0 <= nPP && nPP <= int(newPivot.size()) - 2) {
                        PtoP = std::llround(nCr[newPivot.size() - 2][nPP]);
                        PlusECDSet_ST_V8::processEdgePairs(newPivot, PtoP, addW);
                    }
                    int nKP = int(np) - 1;
                    if (0 <= nKP && nKP <= int(newPivot.size()) - 1) {
                        KtoP = std::llround(nCr[newPivot.size() - 1][nKP]);
                        PlusECDSet_ST_V8::processEdgePairs(newKeepC, newPivot, KtoP, addW);
                    }
                    newPivot.clear(); newKeepC.clear();
                    ...
                    // Note: new sub-leaves from Case B do NOT get leafEdgeInfo
                    // (they use the O(|leaf|^2) fallback if they later die as Case A)
                });
            ...
            if (needPivot <= povit.size()) {
                ...
                PlusECDSet_ST_V8::processEdgePairs(keepC, KtoK, removeW);
            }
            int needPP = int(needPivot) - 2;
            if (0 <= needPP && needPP <= int(povit.size()) - 2) {
                PtoP = std::llround(nCr[povit.size() - 2][needPP]);
                PlusECDSet_ST_V8::processEdgePairs(povit, PtoP, removeW);
            }
            int needKP = int(needPivot) - 1;
            if (0 <= needKP && needKP <= int(povit.size()) - 1) {
                KtoP = std::llround(nCr[povit.size() - 1][needKP]);
                PlusECDSet_ST_V8::processEdgePairs(keepC, povit, KtoP, removeW);
            }
