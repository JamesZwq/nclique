//
// Region Tuple + CPI Counting (V3Fast) + Class-Based Hierarchy (V3HC).
//
// Same peeling / core computation as V3Fast. Hierarchy post-processing
// replaced by a class-level DSU (each class is one node; each FM MC is
// one node) instead of the tuple-level DSU used in V3H.
//
// Rationale: class symmetry (Thm 4.4) implies all vertices of a class
// share the same alive/dead status at every level and belong to the
// same s-connected component. Hence class is the finest legitimate
// DSU atom for hierarchy construction.
//
// Union rules (at each level, descending):
//   (A) for each alive tuple τ, union all classes in τ.key;
//   (B) for each alive tuple τ and each CPI path P covering τ,
//       union any one class of τ with pathClassRep[P];
//   (C) for each alive region M (mcCoreVal(M) ≥ level),
//       union all classes in classesInRegion[M];
//   (D) FM MCs activate as isolated nodes.
//
...
NucleusCoreDecompositionRClique_RegionCPI_HierarchyClass(
    ...
    // --- Post-peeling: class-based hierarchy (V3HC) ---
    //
    // DSU atom = class. By class symmetry (Thm 4.4) + automorphism argument,
    // all vertices of a class share the same alive/dead status at every level
    // and fall in the same s-connected component. FM MCs activate as isolated
    // components. Tuples and regions act only as "union triggers" — they emit
    // union operations among class nodes but are NOT DSU nodes themselves.
    //
    ...
    enum HierEvType { EV_TUPLE = 0, EV_REGION = 1, EV_FM = 2 };
    ...
    // Every region with mcCoreVal > 0 can union its classes (Rule C).
    for (daf::Size rid = 0; rid < numRegions; ++rid)
        if (mcCoreVal[rid] > 0 && !classesInRegion[rid].empty())
            events.push_back({mcCoreVal[rid], EV_REGION, rid});
    for (daf::Size mi = 0; mi < mergedMCs.size(); ++mi)
        if (mergedMCs[mi].coreVal > 0)
            events.push_back({mergedMCs[mi].coreVal, EV_FM, mi});
    // At the same level, process tuples before regions so tuple-induced unions
    // can happen before region-induced unions if both fire at the same core.
    std::sort(events.begin(), events.end(), [](const HierEv &a, const HierEv &b) {
        if (a.level != b.level) return a.level > b.level;
        return a.type < b.type;
    });

    // DSU over [classes | FM MCs]. Each class is one node; each FM MC is one node.
    const daf::Size nodeBase_fm = numClasses;
    const daf::Size totalNodes  = numClasses + mergedMCs.size();
    ...
    const char* dumpPath = getenv("V3H_DUMP_MEMBERS");
    std::ofstream dumpFile;
    if (dumpPath && *dumpPath) {
        dumpFile.open(dumpPath);
        dumpFile << "[\n";
    }
    bool firstLevelDumped = true;
    ...
    while (i < events.size()) {
        ...
        while (i < events.size() && events[i].level == level) {
            ...
            if (ev.type == EV_TUPLE) {
                ...
                daf::Size rid = ev.idx;
                const auto &cs = classesInRegion[rid];
                if (cs.empty()) continue;
                // Activate and chain-union all classes in classesInRegion[M] (Rule C).
                daf::Size anchor = cs[0];
                activateClass(anchor);
        ...
        // Per-level member dump: for each active DSU root, collect the member
        // vertex list by walking all active nodes (classes + FM MCs) in its
        // component. Emits JSON lines to V3H_DUMP_MEMBERS file path.
        if (dumpFile.is_open()) {
            std::unordered_map<int, std::vector<daf::Size>> rootVerts;
            rootVerts.reserve(sizes.size() * 2);
            for (int n = 0; n < (int)totalNodes; ++n) {
                if (!active[n]) continue;
                int root = dsu.find(n);
                if (compVertCount[root] <= 0) continue;
                auto &bucket = rootVerts[root];
                if ((daf::Size)n < numClasses) {
                    for (auto v : classVerts[n]) bucket.push_back(v);
                } else {
                    daf::Size mi = (daf::Size)n - nodeBase_fm;
                    for (auto v : mergedMCs[mi].verts) bucket.push_back(v);
                }
            }
            // Sort components by size descending, and cap at top 20.
            std::vector<std::pair<int,std::vector<daf::Size>*>> comps;
            comps.reserve(rootVerts.size());
            for (auto &kv : rootVerts) comps.emplace_back((int)kv.second.size(), &kv.second);
            std::sort(comps.begin(), comps.end(),
                      [](auto &a, auto &b){ return a.first > b.first; });
            if (!firstLevelDumped) dumpFile << ",\n";
            firstLevelDumped = false;
            dumpFile << "  {\"level\": " << std::fixed << std::setprecision(0) << level
                     << ", \"components\": [";
            size_t shown = std::min<size_t>(20, comps.size());
            for (size_t j = 0; j < shown; ++j) {
                if (j) dumpFile << ", ";
                dumpFile << "{\"size\": " << comps[j].first << ", \"vertices\": [";
                auto &vs = *comps[j].second;
    ...
    std::cout << "  Hierarchy post-processing (class-based): " << hierMs << " ms ("
