...
void runAndPrintStatsCliques(LinkedList **adjListLinked,
    ...
    if (T == 'A') {
        ...
        printf("%lf,%lf,%lf,%lf,%d,%d\n\n", (double) (end - start) / (double) (CLOCKS_PER_SEC), *nCalls, *sumP, *sqP,
               max_k_in, deg);
        if ((flag_d == 1) || (flag_d == 2)) {
            fprintf(fp, "time,nCalls,sumP,sqP,max_k,degen\n");
            fprintf(fp, "%lf,%lf,%lf,%lf,%d,%d\n\n", (double) (end - start) / (double) (CLOCKS_PER_SEC), *nCalls, *sumP,
                    *sqP, max_k_in, deg);
        }
        ...
        // printArray(CSCindex, n + 1);
        // printArray(CSCedges, m);
        // printArray(cliqueCounts, m * (max_k + 1));
