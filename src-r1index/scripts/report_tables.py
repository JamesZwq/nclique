#!/usr/bin/env python3
"""Print the result tables from evidence JSON files.  By default reads the research evidence directory; pass another
directory as the first argument (for example scripts/, where run_final.py writes)."""
import json
from pathlib import Path

import sys
HERE = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else Path(__file__).resolve().parents[2] / 'research' / 'r1_skyline_index_20260918'
