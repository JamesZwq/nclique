...
def main():
    ...
    logs.mkdir(); binary = SRC / 'build' / 'chain_index_tool'
    rec = {'started': datetime.datetime.now().astimezone().isoformat(), 'binary_sha256': sha(binary), 'git': subprocess.run(['git', 'rev-parse', 'HEAD'], cwd=ROOT, capture_output=True, text=True).stdout.strip(),
           'sources': {str(p.relative_to(ROOT)): sha(p) for p in (SRC / 'chain_index.hpp', SRC / 'chain_index_tool.cpp', SRC / 'hierarchy.hpp', SRC / 'solver' / 'terminal.hpp')}, 'runs': []}
    for g in graphs:
