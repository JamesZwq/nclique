...
HERE = Path(__file__).resolve().parent          # scripts/: evidence is written here
SRC = HERE.parent                                # src-r1index (CMake project)
ROOT = SRC.parent                                # repository root; graph paths are relative to it or absolute
...
def main():
    ...
    record = {'started': datetime.datetime.now().astimezone().isoformat(), 'commands': [],
              'sources': {str(p.relative_to(ROOT)): sha(p) for p in
                          (SRC / 'chain_index.hpp', SRC / 'chain_index_tool.cpp', SRC / 'hierarchy.hpp', SRC / 'CMakeLists.txt', HERE / 'run_final.py')},
              'selftests': {}, 'runs': []}
    ...
    for graph in graphs:
        tag = Path(graph).stem
        command = ['/usr/bin/time', TIME_FLAG, str(SRC / 'build' / 'chain_index_tool'), '--bench', graph, str(cx / f'{tag}.cx')]
        record['commands'].append(command)
