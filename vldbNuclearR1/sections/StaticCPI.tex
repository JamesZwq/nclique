...
%
The parameter $q$ is the number of pivots on $\TPath$ that are in scope for $u$: when $u\in\Vh(\TPath)$ this is the alive-pivot count of $\TPath$; when $u\in\Vp(\TPath)$ this is the alive-pivot count of $\TPath$ excluding $u$ itself, so $u$ fills one of the $\eta_{\TPath}$ pivot slots and the remaining $\eta_{\TPath}-1$ are chosen from $q$.
\end{definition}
