...
%
\textsc{BkEmit} is the per-seed pivot-aware enumeration used by the CPI construction lineage~\cite{Pivoter,NuclearCD}; structurally it is Algorithm~\ref{alg:buildcpi}'s \textsc{ExpandPath} subroutine restricted to a single seed and writing into the thread-local arena $A_t$ instead of storing a mutable recursion tree for later residual updates.
