...
Once \spinStar{} makes peeling counter-only, \cpi{} construction becomes the dominant cost of the pipeline (Figure~\ref{fig:breakdown}).
%
...
%
We observe that the static-\cpi{} invariant makes construction embarrassingly parallel: with no shared mutable state across threads and each $s$-clique uniquely owned by its smallest-rank seed, the standard degeneracy-ordered Bron--Kerbosch template runs lock-free across $T$ threads.
...
%
The builder therefore only has to emit the path set $\mathcal{P}$; no per-path mutation needs to be coordinated across threads.
...
%
Each path is therefore emitted by exactly one thread, so the per-thread outputs are pairwise disjoint and $\bigcup_t \mathcal{P}_t$ is the same set of paths Algorithm~\ref{alg:buildcpi} would have produced sequentially.
%
The choice of $T$, the partition of seeds, and the in-memory layout used to store $\mathcal{P}$ are implementation parameters; they affect throughput but not the output set.
...
\stitle{Scope.}
The design parallelises construction because construction is the phase exposed by the static-\cpi{} pipeline.
%
It does not claim a lower bound showing that \spinStar{} cannot be parallelised.
%
