...
%
The boundary case $\eta_{\TPath}\!=\!0$ (i.e.\ $|\Vh(\TPath)|\!=\!s$) encodes exactly one $s$-clique, namely the hold set itself; we adopt the convention $\binom{m}{-1}=0$ so the binomial-difference formulas used later return $0$ for any pivot-removal event on such a path.
%
...
\]
for the \emph{vertex--path incidence count} of the \cpi{}, i.e.\ the cost of reading the whole \cpi{} once, in which every runtime and memory bound in this paper is expressed.
%
We also write $\kmax=\max_v\sdeg(v)$.
%
The \cpi{} is built by a degeneracy-ordered Bron--Kerbosch search with Tomita pivoting~\cite{NuclearCD,BronKerbosch,eppstein,tomita}; we reproduce the construction as Algorithm~\ref{alg:buildcpi} because the rest of the paper operates over the emitted paths.
...
Algorithm~\ref{alg:buildcpi} computes a degeneracy ordering of $G$ and orients edges from lower to higher rank, so each $s$-clique has a unique smallest-rank owner (Line~1).
%
For every vertex $v_i$, $\textsc{ExpandPath}$ is invoked with hold set $\{v_i\}$ and forward-neighbour candidate set $N^{+}(v_i)$ (Lines~3-4).
%
...
Once the \cpi{} is built, each vertex's exact $s$-clique support is a sum over the paths through it: every $s$-clique through $v$ is encoded by exactly one path with $v$ either in the hold or in the pivot set, and the count in each case is a single binomial in the path's pivot size.
