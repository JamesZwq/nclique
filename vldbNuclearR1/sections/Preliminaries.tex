...
%
The boundary case $\eta_{\TPath}\!=\!0$ (i.e.\ $|\Vh(\TPath)|\!=\!s$) encodes exactly one $s$-clique --- the hold set itself; we adopt the convention $\binom{m}{-1}=0$ so the binomial-difference formulas used later return $0$ for any pivot-removal event on such a path.
%
...
\]
for the \emph{vertex--path incidence count} of the \cpi{} --- the cost of reading the whole \cpi{} once, in which every runtime and memory bound in this paper is expressed.
%
