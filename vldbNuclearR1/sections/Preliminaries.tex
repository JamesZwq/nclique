...
%
The boundary case $\eta_{\TPath}=0$ (i.e.\ $|\Vh(\TPath)|=s$) encodes exactly one $s$-clique --- the hold set itself.
%
We adopt the convention $\binom{m}{-1}=0$ so the binomial-difference support formulas used later return $0$ for any pivot-removal event on such a path.
%
...
\]
for the \emph{vertex--path incidence count} of the \cpi{}.
%
$\Sig$ is the cost of reading the whole \cpi{} once, and every runtime and memory bound in this paper is expressed in it.
%
...
%
We reproduce the construction as Algorithm~\ref{alg:buildcpi} because the rest of the paper operates over the emitted paths.
...
%
A recursive call returns immediately when the hold set already exceeds $s$ (Line~6).
%
It emits a path when the candidate set is empty and the hold-plus-pivot size meets $s$ (Lines~8-9).
%
Otherwise the Tomita pivot $u_{\mathrm{piv}}$ is the vertex of $P$ with the most forward neighbours inside $P$ (Line~10).
%
Every non-neighbour of $u_{\mathrm{piv}}$ is then recursively expanded: branching on the pivot itself extends the pivot set, branching on any other vertex extends the hold set (Lines~11-16).

\stitle{Reading clique counts off the \cpi.}
Once the \cpi{} is built, each vertex's exact $s$-clique support is a sum over the paths through it.
%
Every $s$-clique through $v$ is encoded by exactly one path, with $v$ either in the hold set or in the pivot set.
%
The count in each case is a single binomial in the path's pivot size.
...
The first sum in Lemma~\ref{lem:supp} is over paths with $v$ in the hold.
%
Every choice of $\eta_{\TPath}$ pivots from $\Vp(\TPath)$ completes $\Vh(\TPath)$ into a distinct $s$-clique through $v$, giving $\binom{|\Vp(\TPath)|}{\eta_{\TPath}}$ cliques per path.
%
The second sum is over paths with $v$ in the pivot set.
%
$v$ fills one of the $\eta_{\TPath}$ pivot slots, and the remaining $\eta_{\TPath}-1$ are chosen from the other $|\Vp(\TPath)|-1$ pivots, giving $\binom{|\Vp(\TPath)|-1}{\eta_{\TPath}-1}$ cliques per path.
%
