...
\]
for the \emph{vertex--path incidence count} of the \cpi{}: every runtime and memory bound in this paper is expressed in $\Sig$, the cost of reading the whole \cpi{} once.
%
...
The first sum in Lemma~\ref{lem:supp} is over paths with $v$ in the hold: every choice of $\eta_{\TPath}$ pivots from $\Vp(\TPath)$ completes $\Vh(\TPath)$ into a distinct $s$-clique through $v$, giving $\binom{|\Vp(\TPath)|}{\eta_{\TPath}}$ cliques per path.
%
The second sum in Lemma~\ref{lem:supp} is over paths with $v$ in the pivot set: $v$ fills one of the $\eta_{\TPath}$ pivot slots, and the remaining $\eta_{\TPath}-1$ are chosen from the other $|\Vp(\TPath)|-1$ pivots, giving $\binom{|\Vp(\TPath)|-1}{\eta_{\TPath}-1}$ cliques per path.
%
Every $\sdeg(v)$ in the rest of the paper is evaluated through Lemma~\ref{lem:supp}, never by listing cliques.
