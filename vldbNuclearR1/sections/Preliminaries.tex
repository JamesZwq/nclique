...
%
For every vertex $v_i$, $\textsc{ExpandPath}$ is invoked with hold set $\{v_i\}$ and forward-neighbor candidate set $N^{+}(v_i)$ (Lines~3-4).
%
...
%
Otherwise the Tomita pivot $u_{\mathrm{piv}}$ is the vertex of $P$ with the most forward neighbors inside $P$ (Line~10), and every non-neighbor of $u_{\mathrm{piv}}$ is recursively expanded: branching on the pivot itself extends the pivot set, branching on any other vertex extends the hold set (Lines~11-16).
