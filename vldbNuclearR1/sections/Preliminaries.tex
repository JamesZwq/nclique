...
%
A recursive call returns immediately when the hold set already exceeds $s$ (Line~7), and emits a path when the candidate set is empty and the hold-plus-pivot size meets $s$ (Lines~8-9).
%
Otherwise the Tomita pivot $u_{\mathrm{piv}}$ is the vertex of $P$ with the most forward neighbours inside $P$ (Line~10), and every non-neighbour of $u_{\mathrm{piv}}$ is recursively expanded: branching on the pivot itself extends the pivot set, branching on any other vertex extends the hold set (Lines~11-14).
