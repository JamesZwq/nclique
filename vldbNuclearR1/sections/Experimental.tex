...
%
The ten primary benchmark graphs range from \(\sim\)\(0.9\)M edges (\texttt{com-amazon}) to \(117\)M edges (\texttt{com-orkut}).
%
\texttt{com-friendster} (\(1.806\)B edges) is added separately for the billion-edge scalability study in Section~\ref{sec:exp-friendster}; the ten-graph sweep already costs days of single-thread wall time, and including \texttt{com-friendster} in the matched-cell comparison is infeasible because \cbnd{} cannot complete on it.
%
The ten-graph benchmark sweep covers \(762\) \((graph,s)\) configurations (Table~\ref{tab:datasets}); \(307\) of these are matched cells where both \spinStar{} and \cbnd{} complete successfully, and the headline speedup numbers are reported over the matched subset.

\begin{table}[t]
\centering
\caption{Datasets used in Section~\ref{sec:exp}. The first ten graphs form the matched-cell sweep; \texttt{com-friendster} is the billion-edge graph used in the standalone scalability study (Section~\ref{sec:exp-friendster}).
%
The last column counts \((graph,s)\) configurations exercised on each graph.}
\label{tab:datasets}
\small
\begin{tabular}{lrrr}
\toprule
...
\midrule
\textbf{Subtotal} \emph{(matched sweep)} & & & \(\mathbf{762}\) \\
\midrule
\texttt{com-friendster}~\cite{communityFinder} & \(65{,}608{,}366\) & \(1{,}806{,}067{,}135\) & \(10\) \\
\bottomrule
\end{tabular}
\end{table}
