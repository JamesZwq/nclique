...
%
The benchmark sweep covers \(762\) \((graph,s)\) configurations across the ten graphs (Table~\ref{tab:datasets}); \(307\) of these are matched cells where both \spinStar{} and \cbnd{} produce an OK result, and the headline speedup numbers are reported over the matched subset.

\begin{table}[t]
\centering
\caption{Datasets used in Section~\ref{sec:exp}. The last column gives the number of matched successful \((graph,s)\) configurations in the benchmark sweep.}
\label{tab:datasets}
\small
\begin{tabular}{lrrr}
\toprule
...
%
The end-to-end wall-clock speedup is smaller (gmean \(1.10\times\)) because the shared SDCT construction phase dominates total wall time once peeling is cheap --- exactly the bottleneck shift that motivates \paraBuildCPI{} in Section~\ref{sec:parabuild}.
%
...
%
On those cells, \spinStar{} has \(14.62\times\) geometric-mean peel-time speedup over \cbnd{} and \(44.76\times\) maximum.
%
