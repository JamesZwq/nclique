...
\spinStar{} is the event-driven optimization of \spin{}.
%
...
\item \textbf{\spin{} and \spinStar{} over the static index.} We formulate \spin{} as the direct threshold-support iteration on \cpi{} witness counts, and \spinStar{} as the event-driven optimization that computes the same fixed point without repeated global recomputations (Algorithms~\ref{alg:localH} and~\ref{alg:peelH}, Lemma~\ref{lem:equivalence}).
...
\item \textbf{Experimental and application evidence.} We compare against the mutable-\cpi{} baseline, report runtime and memory on \(762\) matched benchmark cells, measure phase behavior and parallel construction, and use case studies to evaluate whether the \(r=1\) specialization is practically useful.
\end{itemize}
