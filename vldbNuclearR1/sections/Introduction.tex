...
\cbnd{}'s per-path vertex hash sets, reverse vertex-to-path map, and live Bron--Kerbosch recursion tree together cost roughly $88\Sig$ bytes (Section~\ref{sec:exp}, Table~\ref{tab:mem}) — close to an order of magnitude more than the $10\Sig$-byte budget of the static layout we develop here.
%
The geometric-mean memory-usage ratio (\cbnd{} over \spinStar{}) is $1.55\times$ on the matched cells and $3.92\times$ at maximum; on \texttt{com-friendster} (the largest SNAP graph), the overhead pushes \cbnd{} past the $503$ GB available at every $s$, so the algorithm never completes on that graph.
...
%
We address the three challenges with the following contributions, and along the way derive two solvers (\spin{} and \spinStar{}) for computing core values over the static index.
