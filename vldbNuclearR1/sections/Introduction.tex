...
%
The classical $k$-core uses edges as the witness, but many graph mining tasks need a stronger witness because a group may pass an edge-degree threshold while containing few triangles, $4$-cliques, or larger coordinated patterns.
%
The \emph{$(1,s)$-nucleus decomposition}~\cite{nucleusSariyuce14} gives this vertex-centric higher-order ranking: for a fixed $s$, it assigns each vertex the largest value $k$ for which the vertex belongs to a maximal region where every vertex participates in at least $k$ internal $s$-cliques.
%
...
The remainder follows the dependency chain: Sections~\ref{sec:prelim}--\ref{sec:baseline} define the problem and the mutable baseline; Sections~\ref{sec:static}--\ref{sec:buildhier} derive the static-\cpi{} pipeline; Sections~\ref{sec:exp}--\ref{sec:casestudy} report performance and applications; Sections~\ref{sec:related}--\ref{sec:conclusion} discuss related work and conclude.
