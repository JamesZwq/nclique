...
%
The geometric-mean memory ratio \cbnd{}/\spinStar{} is $1.55\times$ on the matched cells and $3.92\times$ at maximum; on \texttt{com-friendster} (the largest SNAP graph) the overhead pushes \cbnd{} past the $503$ GB available at every $s$, so the algorithm never completes on that graph.
...
%
This collapses the per-incidence space cost from $\approx 88\Sig$ bytes to $\approx 10\Sig$ bytes (Table~\ref{tab:mem}); the measured geometric-mean memory ratio \cbnd{}/\spinStar{} is $1.55\times$ over $307$ matched cells, and \texttt{com-friendster} (the largest SNAP graph) becomes decomposable for $s\in\{2,\ldots,6,11\}$ on a $503$ GB machine on which \cbnd{} cannot start.
