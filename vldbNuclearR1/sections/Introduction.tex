...
\sstitle{Compact static layout (solves Limitation 2).}
We replace \cbnd{}'s per-path vertex hash sets, reverse vertex-to-path map, and live Bron--Kerbosch recursion tree with two flat dual-CSR arrays that store the path-to-vertex and vertex-to-path incidences directly, plus one integer counter per path.
%
Per-incidence memory drops from $\approx 88\Sig$ bytes to $\approx 10\Sig$ bytes (Table~\ref{tab:mem}); the measured geometric-mean memory-usage ratio \cbnd{}/\spinStar{} is $1.55\times$ over $307$ matched cells, and the new layout makes \texttt{com-friendster} (the largest SNAP graph) tractable on the available $503$ GB at every $s\in\{2,\ldots,6,11\}$ on which \cbnd{} cannot start.
...
%
This gives up to $23.4\times$ construction speedup at $T=24$ threads, eliminating the construction-side bottleneck that remains after peeling becomes counter-only.
