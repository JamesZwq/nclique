...
%
This section organises the algorithms around that single primitive.
%
...
%
The waste has a local structure: a support value can change only when vertices leave the supporting set, and such departures affect only the cliques sitting on paths through the departing vertices.
%
...
\While{$R\ne\emptyset$}{
    ...
    \ForEach{$u\in R$ \emph{with} $\Delta_v(u)\ne 0$}{
        $\supArr[u]\gets\supArr[u]-\Delta_v(u)$\tcp*{$\Delta_v(u)$ realised by per-path losses (Lemma~\ref{lem:changeonly-cpi})}
...
%
At each iteration it finalises the unfinalised vertex of minimum clipped support $\max\{k_{\mathrm{cur}},\supArr[u]\}$, then removes that vertex's set-support contribution from the residual graph.
%
...
The bound is sharp in $\Sig$ but parameterised by $U$; a straightforward incidence argument closes the worst case.
...
%
Thus the affected vertices are the co-path neighbours of $v$ in the static \cpi{}.
%
