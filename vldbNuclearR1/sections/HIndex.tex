...
The static \cpi{} evaluates $\setSup_R(v)$ as a sum of per-path contributions: every encoded $s$-clique sits on a unique path $\TPath$, and that clique counts iff its other $s\!-\!1$ vertices lie in $R$, i.e.\ the path's hold set excluding $v$ is in $R$ and the $\eta_{\TPath}$ pivots (excluding $v$, if $v$ is itself a pivot) are drawn from $\Vp(\TPath)\cap R$.
...
For a fixed $R$ represented by membership bits, Lemma~\ref{lem:set-support-eval} evaluates $\setSup_R(v)$ in time $O(w_v)$ by scanning each path through $v$ once, where $w_v=\sum_{\TPath\ni v}(|\Vh(\TPath)|+|\Vp(\TPath)|)$.
%
\spin{} (Section~\ref{sec:localh}) needs many threshold counts $\setSup_{R_h(k)}(v)$ in one call to $\Phi_h(v)$; sorting the vertex values on each scanned path once and reading the threshold counts off prefix sums produces all of $\setSup_{R_h(0)}(v),\ldots,\setSup_{R_h(h[v])}(v)$ in a single pass per path, at per-call cost $O(w_v\log L_{\max})$ with $L_{\max}=\max_{\TPath}(|\Vh(\TPath)|+|\Vp(\TPath)|)$.
%
Either evaluation reads the static \cpi{} and the set $R$ without mutating any path.
