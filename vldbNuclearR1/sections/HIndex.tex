...
%
\spinStar{} (Section~\ref{sec:peelh}) is what the specification becomes once we notice that most of that recomputation is redundant: maintain $\setSup_R$ on a single mutable residual set and update only the support losses caused by each deletion.
%
...
Anchoring $v$ outside $R$ is what lets a single operator serve both algorithms: \spin{} queries $\setSup_R$ on threshold-induced sets where $v$ may sit on the boundary, and \spinStar{} maintains $\setSup_R$ on a residual set where $v$ always lies in $R$.
%
What remains is to evaluate $\setSup_R(v)$ without enumerating cliques.

The static \cpi{} delivers the evaluation locally.
%
Every encoded $s$-clique sits on a unique path $\TPath$, and that clique counts toward $\setSup_R(v)$ exactly when its other $s\!-\!1$ vertices lie in $R$, i.e.\ the path's hold set excluding $v$ is in $R$ and the $\eta_{\TPath}$ pivots (excluding $v$, if $v$ is itself a pivot) are drawn from $\Vp(\TPath)\cap R$.
%
...
The most direct route from $\setSup_R$ to $\kappa_s$ is self-referential: vertex $v$ deserves level $k$ exactly when $k$ of its $s$-cliques are supported by other vertices that also deserve level $k$.
%
...
%
The initial vector pointwise dominates the final core values and each pass only lowers entries, so the iteration is monotone, terminates, and converges to a vector that serves as the canonical semantic anchor for any static-\cpi{} core-value solver.
...
\spin{} is clean as a specification but wasteful as an algorithm.
%
Between two passes, most vertex values barely move, yet \spin{} re-derives every $\setSup_{R_h(k)}(v)$ from scratch on a freshly induced set (Remark~\ref{rem:spin-ineff}).
%
The redundant work has a sharp structure: the only changes between passes are the lowerings of a few vertex values, and each such change touches only the cliques sitting on paths through the demoted vertex.
%
\spinStar{} converts that observation into an algorithm by replacing the recompute-and-replace cycle with \emph{change-only peeling} on a single mutable set.
...
%
The clip lifts the queue value when the residual support has already fallen below the current level, keeping the extracted-key sequence monotone non-decreasing; each pop's $k$ is therefore the popped vertex's final $\kappa_s$, and the whole peel completes in a single pass.
...
%
So the affected $u$ are the co-path neighbours of $v$ in the static \cpi{}, each $\Delta_v(u)$ decomposes into a sum over the shared paths, and every per-path term is a binomial difference computable in $O(1)$.
