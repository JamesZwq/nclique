% command.tex -- macros for vldbNuclearR1
% Mirrors NuclearCD/command.tex (slim subset)

% --- inline-heading macros (NuclearCD style) ---
\newcommand{\stitle}[1]{{\noindent\bf{#1}}}
\newcommand{\sstitle}[1]{{\noindent\textit{\underline{#1}}}}
\newcommand{\ssstitle}[1]{{\noindent\textit{#1}}}

% --- algorithm name macros (use \kw for sans-serif keyword style) ---
\newcommand{\kw}[1]{{\ensuremath{\mathsf{#1}}}\xspace}
\newcommand{\bkw}[1]{{\ensuremath{\mathsf{\textbf{#1}}}}\xspace}

\newcommand{\cpi}{\kw{CPI}}
\newcommand{\sct}{\kw{SCT}}
% --- main core-value algorithms ---
% Umbrella family name SPIN (Static Path Index for Nucleus decomposition):
%   \spin     = direct fixed-point iteration over the static CPI
%   \spinStar = the same fixed point computed by single-pass bucket-queue peeling
\newcommand{\spin}{\kw{SPIN}}
\newcommand{\spinStar}{\ensuremath{\mathsf{SPIN}^{\star}}\xspace}
\newcommand{\paraBuild}{\kw{ParaBuild}}
\newcommand{\paraBuildCPI}{\paraBuild}  % legacy alias
\newcommand{\buildHier}{\kw{BuildHier}}
% Shared path-local counting kernel (used by both SPIN and SPIN*).
\newcommand{\pathContrib}{\kw{PathContrib}}
% Differential form of the kernel: support loss for one path event.
\newcommand{\pathDelta}{\kw{PathDelta}}
% SPIN's full threshold-support recomputation per vertex.
\newcommand{\thresholdUpdate}{\kw{ThresholdUpdate}}
% Residual-support array maintained by SPIN*.
\newcommand{\supArr}{\mathrm{sup}}
\newcommand{\keyArr}{\mathrm{key}}
% Comparison baseline (general (r,s)-nucleus decomposition restricted to r=1)
\newcommand{\cbnd}{\kw{CND}}

% --- path notation: single CPI path denoted by \TPath (italic P), optional subscript ---
% Plain italic P (not \mathcal{P}) so a single path differs visibly from
% the CPI set \mathcal{P} used in sums like \sum_{\TPath\in\mathcal{P}}.
\makeatletter
\def\TPath{\@ifnextchar\bgroup{\TPathWithArg}{\TPathNoArg}}
\def\TPathWithArg#1{P_{#1}}
\def\TPathNoArg{P}
\makeatother

% V_h / V_p as standalone subscript symbols; written inline as V_h(\TPath)
\newcommand{\Vh}{V_h}
\newcommand{\Vp}{V_p}

% --- support / index size ---
% Sig = total vertex-path incidence count = sum_{P in P}(|V_h(P)|+|V_p(P)|).
% Use the italic Sigma variant so it looks similar to the summation symbol
% (capital Greek Sigma) but is visually distinct in math mode.
\newcommand{\Sig}{\varSigma}
\newcommand{\sdeg}{\deg^{(s)}}     % s-clique support of a vertex
\newcommand{\kmax}{\kappa_{\max}}

% --- theorem-like environments (shared counter, reset per section) ---
\theoremstyle{plain}
\newtheorem{theorem}{Theorem}[section]
\newtheorem{lemma}[theorem]{Lemma}
\newtheorem{corollary}[theorem]{Corollary}
\theoremstyle{definition}
\newtheorem{definition}[theorem]{Definition}
\theoremstyle{remark}
\newtheorem{example}[theorem]{Example}
\newtheorem{observation}[theorem]{Observation}
\newtheorem{remark}[theorem]{Remark}

% --- reference shorthand ---
\newcommand{\refsec}[1]{Section~\ref{sec:#1}}
\newcommand{\reffig}[1]{Fig.~\ref{fig:#1}}
\newcommand{\reftable}[1]{Table~\ref{tab:#1}}
\newcommand{\refalg}[1]{Algorithm~\ref{alg:#1}}
\newcommand{\refdef}[1]{Definition~\ref{def:#1}}
\newcommand{\refthm}[1]{Theorem~\ref{thm:#1}}
\newcommand{\reflem}[1]{Lemma~\ref{lem:#1}}
\newcommand{\refex}[1]{Example~\ref{ex:#1}}

% --- hold/pivot styling (sans-serif, not bold) ---
\newcommand{\hold}{\textsf{hold}\xspace}
\newcommand{\pivot}{\textsf{pivot}\xspace}
\newcommand{\holds}{\textsf{holds}\xspace}
\newcommand{\pivots}{\textsf{pivots}\xspace}

% --- algorithm pseudocode font size (matches NuclearCD) ---
\newcommand{\algfontsize}{\fontsize{7pt}{7.5pt}\selectfont}

% --- experimental-section auto-counter (matches NuclearCD's Exp-N style) ---
\newcounter{expcounter}
\setcounter{expcounter}{0}
\newcounter{expsubcounter}[expcounter]
\setcounter{expsubcounter}{0}
\newcommand{\expsection}[1]{%
    \refstepcounter{expcounter}%
    \setcounter{expsubcounter}{0}%
    \stitle{Exp-\theexpcounter: #1.}%
}
\newcommand{\expsubsection}[1]{%
    \stepcounter{expsubcounter}%
    \ssstitle{Exp-\theexpcounter.\theexpsubcounter: #1.}%
}
