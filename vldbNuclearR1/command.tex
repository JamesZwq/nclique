...
% Set support: the primary high-level operator shared by SPIN and SPIN*.
% S_R(v) = number of s-cliques containing v in G[R].
\newcommand{\setSup}{S}
% Low-level path-local binomial primitive used by the CPI evaluation lemma.
\newcommand{\pathContrib}{\kw{PathContrib}}
% Per-path differential used in the implementation lemma for change-only updates.
\newcommand{\pathDelta}{\kw{PathDelta}}
% Residual-support and priority-queue-key arrays maintained by SPIN*.
\newcommand{\supArr}{\mathrm{sup}}
\newcommand{\keyArr}{\mathrm{key}}
...
% --- support / index size ---
% Sig = total vertex-path incidence count = sum_{P in P}(|V_h(P)|+|V_p(P)|).
% Use the italic Sigma variant so it looks similar to the summation symbol
% (capital Greek Sigma) but is visually distinct in math mode.
