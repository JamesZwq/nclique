...
namespace CDSetRSRef {
    ...
    struct CompareRClique {
        ...
        bool operator()(daf::Size const &a, daf::Size const &b) const {
            ...
            return a < b; // deterministic tie-break for comparison
        }
    };
    ...
    // Parallel counting after index is built (same result as countingPerRClique).
...
std::vector<std::pair<std::vector<daf::Size>, int> > NucleusCoreDecompositionRCliqueRef(
    ...
    auto countingRClique = daf::timeCount("countingPerRClique",
                                          [&]() {
                                              return CDSetRSRef::countingPerRCliqueParallel(
    ...
    // CDSetRSRef::printEdgeCore(edgeGraph, countingKE);
    ...
    // CDSetRSRef::printEdgeCore(edgeGraph, degreeE);
#endif
    ...
    CDSetRSRef::DHeap heap{CDSetRSRef::CompareRClique(countingRClique.data())};
    heap.reserve(cliqueIndex.size());

    std::vector<CDSetRSRef::DHeap::handle_type> heapHandles(cliqueIndex.size());
...
#ifdef _OPENMP
        // Parallel over leaves: compute newLeaves, incr, decr per leaf (thread-local BK map)
#pragma omp parallel
        {
            ...
            for (daf::Size idx = 0; idx < changedLeaf.size(); ++idx) {
                ...
                for (const auto &newLeaf : leafResults[idx].newLeaves) {
                    ...
                    daf::enumerateCombinations(newLeaf, r, [&](const daf::StaticVector<TreeGraphNode> &rclique) {
                        daf::CliqueSize subNumPovit = 0;
                        for (const auto &node : rclique) if (node.isPivot) subNumPovit++;
                        if (subNumPovit <= needPivot && newPivotC - subNumPovit >= 0 &&
                            newPivotC - subNumPovit < 1001 && needPivot - subNumPovit >= 0 && needPivot - subNumPovit < 401) {
                ...
                daf::enumerateCombinations(leaf, r, [&](const daf::StaticVector<TreeGraphNode> &clique) {
                    ...
                    double ncrValue = nCr[pivotC - subNumPovit][s - keepC - subNumPovit];
                    leafResults[idx].decr.emplace_back(cliqueIndexId, ncrValue);
                    return true;
                });
            }
        }
...
#pragma omp parallel for schedule(dynamic, 1024)
        ...
        for (auto leafId : changedLeaf) {
            ...
            bkRmClique::removeRClique(leaf, mapped, r, s, [&](const bkRmClique::Bitset &c, const bkRmClique::Bitset &pivots) {
                auto newLeaf = bkRmClique::coverToVertex(c, pivots, leaf);
                DEBUG_BREAK_IF(newLeaf.size() < s);
                newLeavesContent.push_back(std::move(newLeaf));
            });
            ...
            for (const auto &newLeaf : newLeavesContent) {
                daf::CliqueSize newPivotC = 0, newKeepC = 0;
                ...
                daf::enumerateCombinations(newLeaf, r, [&](const daf::StaticVector<TreeGraphNode> &rclique) {
                    ...
                    if (subNumPovit <= needPivot && newPivotC - subNumPovit >= 0 && newPivotC - subNumPovit < 1001 &&
                        ...
                        double ncrValue = nCr[newPivotC - subNumPovit][needPivot - subNumPovit];
                        countingRClique[cliqueIndex.byClique(rclique)] += ncrValue;
                    }
                    return true;
                });
            }
...
void print_clique_ref(const Bitset &bs) {
    std::cout << '[';
    bool first = true;
