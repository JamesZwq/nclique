...
%
The index has three parts, drawn in Figure~\ref{fig:layout} for the running example: the vertex-to-chain map, one record per chain, and for every size one tree over the chains with an array of runs (Section~\ref{sec:runs}).
%
...
%
The chains are numbered $0$ to $C-1$ in the order of Section~\ref{sec:order}, and the interval of a chain runs from its first label to the first label of the next chain.
%
...
In Figure~\ref{fig:layout}, the chains $\{b_1,b_2,b_3\}$, $\{a_1,\dots,a_5\}$, $\{u_1,\dots,w_3\}$ and $\{x\}$ are numbered $0$ to $3$ with the labels $[0,3)$, $[3,8)$, $[8,14)$ and $[14,15)$, so the chain starts are $0$, $3$, $8$ and $14$.
%
The vertex $u_1$, with label $8$, has three starts at or before it and lies in chain $2$.
\end{example}
...
%
At each node, the traversal lists the chains whose own node it is and the subtrees of its children in increasing order of the smallest chain rank each contains, a chain counting as its own rank.
%
It orders the roots in the same way.
%
...
%
Two chains listed one after the other whose ranks differ by one own adjacent label intervals.
%
Maximal sequences of such chains are merged into \textit{runs}, numbered $0$ to $R-1$, and run $r$ covers the labels from $\mathrm{lo}(r)$ to $\mathrm{hi}(r)$, the latter excluded.
%
...
%
So $L$ has the two properties at the level of the floor and lies inside a core of size $s$ at that level.
...
In Figure~\ref{fig:example}, $\core{2}{a_i}=4=\binom{4}{1}$ with $\omg{a_i}=5$ and $\core{2}{b_i}=3=\binom{3}{1}$ with $\omg{b_i}=4$, so $\sig{a_i}=\sig{b_i}=2$ and no value of $A$ or $B$ is stored.
%
The vertex $x$ has $\core{2}{x}=4>\binom{3}{1}$ and $\core{3}{x}=3=\binom{3}{2}$, so $\sig{x}=3$ and its residue is the value $4$.
%
The six vertices $u_1,\dots,w_3$, with $\omega=3$, never reach the floor, so $\sigma=4$ and both values are residues.
%
...
Let a word hold one label, one node or one value, and recall that $C$ is the number of chains.
%
...
%
The layers take a constant number of words per canonical node (parent, top, subtree size and entry) and two labels per run.
%
