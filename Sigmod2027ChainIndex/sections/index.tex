...
%
The index has three parts, drawn in Figure~\ref{fig:layout} for the running example: the vertex-to-chain map, one record per chain, and for every size one tree over the chains with an array of runs (Section~\ref{sec:runs}), the \textit{layer} of that size.
...
%
The map takes $n+o(n)$ bits and $C$ words for the first labels; the chain records take $2C$ words for $\omega$ and $\sigma$, one word per (chain, size) pair for the trajectories, and one word per residue; the layers take a constant number of words per canonical node (parent, top, subtree size and entry) and two labels per run.
%
\strees takes one word per (vertex, size) pair for its arrays, as many for its own-node pointers, a value for each of these pairs, and a constant number of words per canonical node, so the saving is in the stored units, $\sum_v(\omg{v}-1)$ (vertex, size) pairs against $\sum_c(\omega(c)-1)$ (chain, size) pairs, and in the values, the residues against a value per pair.
%
On the running example the index stores $3$ values, $8$ words for $\omega$ and $\sigma$ and $12$ trajectory words, while \strees stores $44$ values and $44$ own-node pointers.
