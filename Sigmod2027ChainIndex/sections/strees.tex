...
%
$\Tree{3}$ has two roots and no other node: $N=B\cup\{u_1,\dots,w_3\}$ with top $3$ and $A$ with top $6$; no triangle joins them, because the edge $a_5b_3$ lies in none.
%
...
%
Every node stores its top, its parent and the two ends of its slice; every vertex stores its own node and its core value.
%
...
%
The arrays therefore hold $\sum_v(\omg{v}-1)$ entries; there are as many own-node pointers, and as many stored values.
%
