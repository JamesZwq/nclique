...
%
The set is connected through these cliques: any two of its vertices are joined by a sequence of its $s$-cliques, consecutive ones sharing a vertex.
%
...
%
Larger sizes demand denser groups.
%
...
%
The clique $A$ is a community of its own at every size.
%
...
%
At size $3$ it is the $23$ albums of one record label, and at size $5$ the six albums a shop would place on one shelf (Section~\ref{sec:exp}).
%
\sstitle{Coordinated groups.}
In a graph of accounts joined when they act on the same posts, accounts that act together on many posts form cliques, and casual interaction rarely forms one~\cite{beutel2013copycatch,hooi2016fraudar}.
%
...
%
The alternative is to keep no index and to decompose the graph at the queried size when the query arrives, seconds to minutes per query on these graphs.
%
...
%
But the depth-first order of one size lists the groups in one order and that of another size in another.
%
...
%
The \emph{own node} of a vertex at a size is its community at its core value.
%
...
%
We call the classes \emph{chains}.
%
...
We give the vertices new ids, their \emph{labels}, so that every chain is one interval, and we order the chains by the depth-first positions of their own nodes, size $2$ first.
%
Every size-$2$ community is then one range of labels (Theorem~\ref{thm:onerange}), and the communities of larger sizes are few ranges (Figure~\ref{fig:regimes}).
%
For each size, the index stores the tree of that size over the chains and the order in which a depth-first traversal lists them, as \emph{runs} of consecutive labels.
%
A community query climbs to the node of the queried level and reads that node's ranges in constant time (Theorem~\ref{thm:retrieval}).
%
Once the core value of a vertex equals the number of cliques through it inside a largest clique containing it, a binomial coefficient of its clique number, it does so at every larger size (Theorem~\ref{thm:tail}).
%
The index stores values only below that size.
%
\sstitle{One clique tree for every size.}
A clique tree~\cite{Pivoter} gives the clique counts of every size at once.
%
The values of one size come from a peel, and each size first replays the removal order of the previous size, exact when a check on clique counts holds (Theorem~\ref{thm:replay}).
...
%
It locates a community in \locmin{} to \locmax{} nanoseconds and lists it in \stmininv{} to \stmaxinv{} times the time of a memory copy of the answer (median \stmedianinv).
%
...
%
On \dblp the size that best matches a community given as ground truth changes from query to query.
%
