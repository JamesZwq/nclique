...
This section answers the first difficulty of Section~\ref{sec:intro}: which vertices an index can store as one group, with one stored record for all of them, for every size, every level and every query.
%
% 2026-09-22 filler pass: one preview sentence.  Cold read: "decide" -> "determine"; "coarsest" -> "largest groups".
We define hierarchy equivalence and prove that the own nodes alone determine it (Theorem~\ref{thm:chains}).
%
Its classes, the chains, are the partition of the vertices into the largest groups that loses no answer (Proposition~\ref{prop:coarsest}).
...
%
The community of $u$ at $(s,\core{s}{u})$ exists, as $\core{s}{u}\ge1$, and contains $u$ and not $v$, since every vertex of a core at level $\core{s}{u}$ has at least that core value.
%
It is then not a union of blocks of $P$, since $u$ and $v$ share a block.
%
...
%
Hence the own nodes agree at every size, so $\traj{u}=\traj{v}$, and $u$ and $v$ share a chain.
\end{proof}
...
\end{lemma}
\begin{proof}
Swapping $u$ and $w$ maps $G$ onto itself, since the two have the same closed neighbourhood, and cores are defined from the graph alone, so it maps cores to cores and preserves every core value.
%
...
%
Otherwise $K=\{u,w\}$, a core having at least $s\ge2$ vertices, and the swap maps it to itself.
%
Hence $u$ and $w$ are hierarchy-equivalent.
%
Twins are adjacent, since $u$ lies in the closed neighbourhood of $w$, so $\omg{u},\omg{w}\ge2$ and Theorem~\ref{thm:chains} puts them in one chain.
\end{proof}
% 2026-09-22 filler pass: "How many chains" merged here; "Relation to other groupings" removed (Section 9 has SGL and EquiTruss).
By Lemma~\ref{lem:twins}, chains are at most as many as twin classes, and on our graphs they are far fewer.
%
