...
test_variant() {
    ...
    for i in 1 2 3; do
        ...
        if [ -n "$nucleus_time" ]; then
            times+=($nucleus_time)
            echo "    Nucleus time: $nucleus_time ms"
        else
...
echo "Starting Optimization Experiments" | tee -a $RESULTS_DIR/experiment.log
echo "Start time: $(date)" | tee -a $RESULTS_DIR/experiment.log
echo "========================================" | tee -a $RESULTS_DIR/experiment.log
echo "" | tee -a $RESULTS_DIR/experiment.log

# 测试基准版本
echo "Testing baseline version..." | tee -a $RESULTS_DIR/experiment.log
test_variant "baseline" "/home/wenqianz/nclique/src/NucleusDecomposition/NucleusCoreDecompositionRemoveScliqueRef.cpp" | tee -a $RESULTS_DIR/experiment.log
...
echo "========================================" | tee -a $RESULTS_DIR/experiment.log
echo "" | tee -a $RESULTS_DIR/experiment.log
echo "Results summary:" | tee -a $RESULTS_DIR/experiment.log
