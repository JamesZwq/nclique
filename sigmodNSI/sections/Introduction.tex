...
On clique-structured graphs, \specnd{} is up to $1{,}600$ times faster than the state-of-the-art per-cell algorithm and uses up to $140$ times less memory; on five graphs the per-cell algorithm cannot complete any spectrum cell within a $300$GB budget while \specnd{} finishes the whole spectrum in under $80$ seconds, and the advantage grows with the clique order.
%
Against per-cell recomputation with our own engine, a certified cell costs $0.07$ seconds where recomputation takes $655$ seconds, a $9{,}364\times$ reduction, and widening the spectrum from six cells to twelve adds under one second.

% ---------------- Figure 1 ----------------
\begin{figure}[t]
  \centering
  \includegraphics[width=0.86\columnwidth]{figures/fig1_running.pdf}
