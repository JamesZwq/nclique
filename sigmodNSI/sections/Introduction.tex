...
We prove that no $r$-clique inside a maximal clique dies before the clique's closed-form level (Theorem~\ref{thm:noearly}), and that every dying $s$-clique is owned by exactly one death event (Theorem~\ref{thm:ownership}); together they let \specnd{} schedule certified deaths at known levels and peel only the residue, with bit-exact output.
%
...
We design \specnd{} (Algorithm~\ref{alg:specnd}), which builds one shared clique-tree structure serving every cell, cold-peels the cheapest cell, certifies each subsequent cell by one integer comparison per pattern, and replays certified deaths at their closed-form levels while peeling only the residue (Theorems~\ref{thm:noearly},~\ref{thm:ownership},~\ref{thm:quotient}).
%
