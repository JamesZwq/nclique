\myaddRA{\subsection{Instantiation II: Edge-Contribution DPCM for \texorpdfstring{$r=2$}{r=2}}}
\label{sec:r2}

For $r{=}2$, DPCM specializes to an \emph{edge-contribution calculus}.
%
The sufficient state of an affected path consists of its current \hold/\pivot partition together with the local removal pattern on that path.
%
This state induces a small exact case split, and the dominant cases admit closed-form contribution updates rather than general residual-graph reconstruction.
%
We start from the structural path-state transition in Theorem~\ref{thm:remove_edge}.
%
...
Thus, we update the path with respect to the pivot endpoint.
%
...
\stitle{DPCM Case Partition.}
Theorem~\ref{thm:remove_edge} yields the exact $r{=}2$ case split required by Theorem~\ref{thm:dpcm-states}.
%
The \hold-\hold case kills the path immediately.
%
The \hold-\pivot and pivot-only removal cases preserve a single path and admit direct support updates.
%
The \pivot-\pivot case is the only branching case, and even there the dominant subcases can still be handled analytically.

\begin{theorem}[Closed-Form Support Delta for Pivot Removal]\label{thm:r2_pivot_delta}
Let $\TPath=(V_p,V_h)$ be an active path with $P=|V_p|$, $K=|V_h|$, and $\mathit{np}=s-K$.
Suppose a peeling round removes $d$ pivots from $\TPath$ while keeping all holds alive and leaving the path feasible, i.e., $\mathit{np}\le P-d$.
Then the support change of each surviving edge $e$ contributed by $\TPath$ depends only on its type:
\[
\Delta_{\TPath}(e)=
\begin{cases}
\binom{P}{\mathit{np}}-\binom{P-d}{\mathit{np}}, & e\subseteq V_h,\\[4pt]
\binom{P-1}{\mathit{np}-1}-\binom{P-d-1}{\mathit{np}-1}, & |e\cap V_h|=1,\\[4pt]
\binom{P-2}{\mathit{np}-2}-\binom{P-d-2}{\mathit{np}-2}, & e\subseteq V_p \text{ and both pivots survive.}
\end{cases}
\]
For any edge incident to a removed pivot, the full original contribution of $\TPath$ is subtracted.
\end{theorem}

\begin{proof}
By Lemma~\ref{lem:clique_count}, the contribution of an edge on $\TPath$ equals the number of ways to choose the remaining pivots needed to form a size-$s$ clique.
%
For a \hold-\hold edge, no pivot is fixed; for a \hold-\pivot edge, one pivot is fixed; for a surviving \pivot-\pivot edge, two pivots are fixed.
%
The contribution in each case is therefore a single binomial term before and after the removal of $d$ pivots, and the delta follows by subtraction.
\end{proof}

\begin{theorem}[Analytical Single-Edge Split]\label{thm:r2_single_split}
Let $\TPath=(V_p,V_h)$ be an active path and let $\{u,v\}\subseteq V_p$ be the only removed \pivot-\pivot edge on $\TPath$.
Then the surviving clique space on $\TPath$ is represented exactly by the two subpaths
\[
\TPath_1=(V_p\setminus\{v\},V_h), \qquad
\TPath_2=(V_p\setminus\{u,v\},V_h\cup\{v\}).
\]
Moreover, the support contribution on each subpath is obtained directly from Lemma~\ref{lem:clique_count}, so no residual-graph search is needed.
\end{theorem}

\begin{proof}
Every surviving size-$s$ clique encoded by $\TPath$ either excludes $v$ or includes $v$ and excludes $u$.
%
The first family is encoded exactly by $\TPath_1$.
%
The second family is encoded exactly by $\TPath_2$, where $v$ is promoted to a hold because every clique in that family must contain it.
%
These two families are disjoint and exhaustive over all cliques on $\TPath$ that avoid the removed edge.
\end{proof}
...
Theorem~\ref{thm:remove_edge} specifies how a single edge deletion updates one clique path.
In peeling, many edges are removed together.
Processing them one by one revisits the same paths repeatedly.
This is most expensive for pivot--pivot deletions, where each deletion may split a path and trigger cascading updates.

We use a \emph{path-local} batched DPCM strategy.
For each affected clique path, we first merge all removed edges that touch this path.
We then handle \hold-\hold deletions by killing the path, and \hold-\pivot / pivot-only deletions through the closed-form update of Theorem~\ref{thm:r2_pivot_delta}.
If exactly one \pivot-\pivot edge remains, we apply Theorem~\ref{thm:r2_single_split}.
Only the residual multi-edge \pivot-\pivot case falls back to a residual graph on $V(P)$ after removing these edges.
Finally, we reconstruct the resulting clique paths from the residual graph in a single pass.
The residual graph is small (bounded by the maximum clique size), so the reconstruction is efficient.
...
\input{sections/algorithms/removeEdge}
...
Cases (1)-(2) follow directly from the hold/pivot rules, and case (3) holds because the residual graph deletes exactly the removed pivot--pivot edges while preserving all remaining edges; Bron--Kerbosch enumeration on this residual graph yields all surviving cliques uniquely.
...
If a \hold-\hold case is triggered, we return in $O(1)$ time.
If the update falls into the closed-form cases of Theorem~\ref{thm:r2_pivot_delta} or the analytical split of Theorem~\ref{thm:r2_single_split}, the update costs only $O(L)$ time.
Only the residual multi-edge \pivot-\pivot case requires the local Bron-Kerbosch-like search on $H$, which costs $O(3^{L/3})$ time and $O(L^2)$ space in the worst case.
