#!/usr/bin/env python3
# Auto-generated for 5513839

STUDENT_ID = "5513839"
STUDENT_NAME = "Xitao Wang"
...
class kCoreBaseStructuralDiversity(object):
    ...
    def process(G, k):
        ...
        for v in range(n):
            ...
            # Special case for k=0: all vertices are in 0-core
            if k == 0:
                tau[v] = kCoreBaseStructuralDiversity._count_connected_components(subgraph_adj, list(range(len(neighbor_list))))
            else:
                ...
                tau[v] = kCoreBaseStructuralDiversity._count_connected_components(subgraph_adj, k_core_vertices)
    ...
    def _find_k_core_vertices(adj_list, k):
        """
        Find all vertices that belong to k-cores in the given graph
        """
        n = len(adj_list)
        if n == 0:
            return []
        ...
        # Initially add vertices with degree < k to removal queue
        for i in range(n):
            if degrees[i] < k:
                queue.append(i)
                removed[i] = True
        ...
        # Return vertices that remain (k-core vertices)
        return [i for i in range(n) if not removed[i]]

    @staticmethod
    def _count_connected_components(adj_list, vertices):
        """
        Count the number of connected components among the given vertices
        """
        if not vertices:
            return 0
