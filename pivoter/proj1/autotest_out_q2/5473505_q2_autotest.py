#!/usr/bin/env python3
# Auto-generated for 5473505

STUDENT_ID = "5473505"
STUDENT_NAME = "Zibo Wei"
            ...
            for i, u in enumerate(neighbors):
                for w in G.adj_list[u]:
                    if w in neighbor_set:
                        nbr_adj[i].append(neighbor_to_idx[w])
    ...
    def _count_k_cores(adj_list, k):
        ...
        for i in range(n):
            if not removed[i] and not visited[i]:
                components += 1
                kCoreBaseStructuralDiversity._dfs_mark_component(adj_list, i, visited, removed)
        
        return components
    
    @staticmethod
    def _dfs_mark_component(adj_list, start, visited, removed):
        stack = [start]
        visited[start] = True
