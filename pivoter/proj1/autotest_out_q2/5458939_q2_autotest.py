#!/usr/bin/env python3
# Auto-generated for 5458939

STUDENT_ID = "5458939"
STUDENT_NAME = "Xiaotian Zou"
...
class kCoreBaseStructuralDiversity(object):
    ...
    def process(G, k: int) -> List[int]:
        ...
        for v in range(n):
            ...
            b = len(neighbours_set)

            if a != b:
                print("Duplicate check:", a, b, v)
                count = Counter(neighbours)
                duplicates = {elem for elem, freq in count.items() if freq > 1}
                print(duplicates)

            # Constructing a neighbour-induced subgraph
            ...
            # Calculate the number of k-cores in the subgraph
            sd[v] = kCoreBaseStructuralDiversity._count_k_core(sub_G, k)

        return sd

    @staticmethod
    def _count_k_core(sub_G: Dict[int, List[int]], k: int) -> int:
        """
        ...
        # BFS finds the number of connected components (i.e. the number of k-cores)
        visited: Set[int] = set()
        k_core_count = 0
