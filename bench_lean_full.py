#!/usr/bin/env python3
"""
SPIN★ Lean (PIVOTER_RUN_ST_V3_LEAN) full sweep on the same 10 graphs as
the main bench. Result rows go to paper_data/bench_lean.csv with the
same schema as bench_local_v4.csv so plot scripts can co-load them.

Resume-friendly: skips (graph, s) cells already at OK status.
Skip-floor: TIMEOUT/OOM at s_min skips s>=s_min for that graph.
Concurrency capped at 4 (same memory profile as SPIN★ V3).

Usage:
    nohup python3 bench_lean_full.py tods2 > /tmp/bench_lean_full.log 2>&1 &
"""
...
TIMEOUT  = 3600
OUTCSV   = Path("paper_data/bench_lean.csv")
LOGDIR   = Path("bench_lean_logs")
...
def main():
    print("=" * 60)
    print(f"  bench_lean_full  {time.strftime('%F %T')}")
    print("=" * 60)
    ...
    for g, smax in graphs:
        for s in range(2, smax + 1):
            ...
            jobs.append(Job(
                ...
                env={"OMP_NUM_THREADS": "1", "PIVOTER_RUN_ST_V3_LEAN": "1"},
                log_path=log_path,
                timeout=TIMEOUT,
            ))
    ...
    def parse_took(log_text: str) -> str:
        import re
        # Match "ST_V3_Lean r=1 (peel) took: ... ms" — uses daf::timeCount label
        m = re.search(r"ST_V3_Lean r=1 \(peel\) took:\s+([\d.]+)\s+ms", log_text)
    ...
    def on_finish(job: Job, status: str, log_text: str, parsed: dict):
        g, s = job.key
        row = {
            "graph": g, "s": s, "algo": "ST_V3_LEAN", "status": status,
